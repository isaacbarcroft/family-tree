...
1. **1.6 Accessibility** — `alt` on memory images, focus traps in modals, semantic HTML on page shells. `TreeNode.tsx` keyboard is already done. Highest-impact remaining gap; older relatives are the target audience.
2. **T-15** — `middleware.ts` for route-level auth. Defense-in-depth on top of RLS.
3. **T-3** — extend `next/image` adoption beyond `ProfileAvatar`.
...
- ~~Structured oral-history prompts (1.4)~~ — shipped 2026-05-17
...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ — Done 2026-05-17

See Completed log. **Open follow-ups:**

- **1.4.a AI-generated follow-up prompts.** Hit Claude API with the user's saved answer to suggest a tailored next question ("You mentioned your father's Studebaker — what else did he drive over the years?"). +4h. Deferred from the original split.
- **1.4.b Self-service prompt authoring.** Today, the `story_prompts` bank is admin-only via RLS. An admin UI to add / edit / deactivate prompts is the right next step once anyone other than Isaac wants to curate. +2h.
- **1.4.c Surface promptId on the answering memory tile.** The `story_prompt_responses` join is recorded but the memory tile doesn't display "Answer to: {prompt body}" yet. +1h.
...
- 2026-05-17 — **1.4 Guided story prompts (first coherent sub-scope).** Migration `20260517_story_prompts.sql` (+ rollback) adds `public.story_prompts (id, body, category, "sortOrder", "isActive", "createdAt")` and `public.story_prompt_responses (id, "promptId", "userId", "memoryId", "createdAt")` with cascade FKs on `public.story_prompts(id)`, `auth.users(id)`, and `public.memories(id)`. `memoryId` is uniquely constrained so a memory can answer at most one prompt. RLS mirrors the allowlist model: any approved user SELECTs from both tables; `story_prompts` writes are admin-only (the bank ships with the migration, future authoring is a deferred admin UI); `story_prompt_responses` INSERTs require `userId = auth.uid()`, DELETEs require owner-or-admin, and UPDATE is not granted (responses are immutable). Indexes on `("isActive")`, `(category)`, `("promptId")`, `("userId")`, `("memoryId")`. Body length is constrained 1 to 500 chars after `btrim`; category is locked to a 7-value enum (`childhood`, `family`, `milestones`, `places`, `food`, `beliefs`, `hobbies`). Seed bank: 90 family-safe prompts spread across all 7 categories (15 each for childhood / family / milestones, 13 for places, 10 for food / beliefs, 12 for hobbies), inserted via a `where not exists` self-join on `body` so re-running the migration is a no-op. New models `src/models/StoryPrompt.ts` and `src/models/StoryPromptResponse.ts`; new constant module `src/constants/storyPrompts.ts` with `STORY_PROMPT_CATEGORIES` + a `STORY_PROMPT_CATEGORY_LABELS` map. New db helpers in `src/lib/db.ts`: `listStoryPrompts()` (`isActive=is.true` + `order=sortOrder.asc`), `listStoryPromptResponsesForUser(userId)`, `addStoryPromptResponse({promptId, userId, memoryId})`. New pure `pickPromptOfTheDay({prompts, answeredPromptIds, userId, today})` in `src/utils/storyPrompt.ts` that filters to active prompts, prefers unanswered ones, falls back to the full active set when every prompt is answered, and hashes `(userId, isoDate)` with FNV-1a to deterministically index into the pool so a refresh during the day does not shuffle the prompt. `src/components/AddMemoryModal.tsx` accepts a new optional `prompt: { id, body }` prop: when set the modal heading flips to "Answer the prompt", a banner labelled "A question for you" renders above the title field, and the title input is pre-filled with the prompt body so the user can edit or save as-is. After a successful `addMemory`, the modal calls `addStoryPromptResponse` to link the new memory back to its prompt; the call is wrapped in a try/catch so a stale `unique("memoryId")` collision or transient RLS failure does not block the save. New `src/components/StoryPromptCard.tsx` renders the home widget: loads the active prompt bank + the viewer's existing responses, picks the prompt of the day, shows the category chip + the prompt body in display-italic, and exposes two CTAs ("Answer in writing" with the `pencil` icon and "Answer with voice" with the `memory` icon) that both open the modal with the prompt context wired in. Card is hidden during loading, when no active prompt exists, and when both fetches fail (graceful degradation). Wired into `src/app/page.tsx` between the lower grid and the "Suggested actions" block, gated on `!isNewUser` so users with 5 or fewer relatives keep the "Getting started" checklist instead. Coverage: 11 migration regex assertions (table shape + category enum + length check + RLS gates on both tables + indexes + unique memoryId + admin-only writes on the prompt bank + immutable responses + seed shape with 50+ rows across all 7 categories + idempotent seed + no destructive ops + rollback drops everything), 9 picker cases (`storyPrompt.test.ts`: empty list, all-inactive, never-inactive-pick, prefers unanswered, full-fallback when all answered, deterministic for same `(userId, today)`, rotation across users, rotation across days, sortOrder-tie stability), 5 db cases (`storyPromptsDb.test.ts`: filter + order, hydration, response filter, response insert body, error propagation), 6 card cases (`storyPromptCard.test.tsx`: empty load, render, modal-open with prompt context, post-save dismiss, prefers unanswered, graceful failure), 4 modal cases (`addMemoryModalPrompt.test.tsx`: banner + title pre-fill, no-prompt heading fallback, response insert after save, no insert without a prompt, non-blocking on response failure). 565 tests pass / 5 skipped (was 522/5; +43); `yarn lint` clean (0/0); `yarn build` green. `SUPABASE_SETUP.md` lists the new migration. Files: `supabase/migrations/20260517_story_prompts.sql` (new), `supabase/migrations/20260517_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/models/StoryPromptResponse.ts` (new), `src/constants/storyPrompts.ts` (new), `src/utils/storyPrompt.ts` (new), `src/lib/db.ts`, `src/components/AddMemoryModal.tsx`, `src/components/StoryPromptCard.tsx` (new), `src/app/page.tsx`, `src/__tests__/storyPromptsMigration.test.ts` (new), `src/__tests__/storyPrompt.test.ts` (new), `src/__tests__/storyPromptsDb.test.ts` (new), `src/__tests__/storyPromptCard.test.tsx` (new), `src/__tests__/addMemoryModalPrompt.test.tsx` (new), `SUPABASE_SETUP.md`, `TODOS.md`. Branch: `claude/keen-newton-19t3O`. Deferred: 1.4.a AI follow-up prompts (+4h), 1.4.b admin authoring UI (+2h), 1.4.c surface "Answer to: {prompt}" on memory tiles (+1h).
