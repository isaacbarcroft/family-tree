...
- 2026-05-25 — **Styling drift — port `ConfirmDialog` + `ProtectedRoute` loading state to paper tokens; delete dead `LoadingSpinner`.** Audit-driven, picks up three of the smallest entries on the 2026-05-18 deferred list (`ConfirmDialog`, `LoadingSpinner`, `ProtectedRoute`). `ConfirmDialog` previously rendered as a `bg-gray-950` pill with `text-red-400` / `text-gray-400` / `hover:bg-gray-800` / `hover:text-white` — a dark blob punched out of the paper-themed person + family cards on `/family-tree` and `/families` (3 call sites). Now the container uses `background: var(--paper)` + `border: 1px solid var(--hairline-strong)` + `box-shadow: var(--shadow-sm)`; the confirm action reads `var(--clay-deep)` with `font-weight: 500` (matching the destructive convention from `MemoryComments`'s "Confirm delete" link); the cancel reads `var(--ink-2)`. Hover lives in `globals.css` as `.confirm-dialog-btn:hover { background: var(--paper-3) !important }` because Tailwind `hover:` utilities can't override an inline-style background (same approach already used by `.ui-btn` and `.memory-reaction-button`). Both buttons keep `type="button"` so they don't submit a parent form. `ProtectedRoute` loading copy moves from `text-gray-400` to `var(--ink-3)` + `.display-italic` so it matches the page-not-found / error fallback styling on `/profile/[id]`. `LoadingSpinner.tsx` had zero imports in `src/` (grep `from.*LoadingSpinner|import LoadingSpinner` returns nothing) — deleted as dead code per the CLAUDE.md "delete unused code completely" rule. Behavior is unchanged in every consumer: `ConfirmDialog` still calls `onConfirm` / `onCancel`, still honors custom `confirmLabel` / `cancelLabel`, still renders the same DOM shape with the same accessible button labels; `ProtectedRoute` still gates children on `user`, still redirects unauthenticated users to `/login`, still shows a loading placeholder during the auth resolve. Coverage: 13 new Vitest cases — 9 in `confirmDialog.test.tsx` (default + custom labels, onConfirm / onCancel wiring, `type="button"` pin, paper-token container regression pin, clay-deep + bold confirm pin, ink-2 cancel pin, `.confirm-dialog-btn` class pin) and 4 in `protectedRoute.test.tsx` (renders children when signed in, shows loading copy while resolving, redirects when no user, ink-3 loading-copy regression pin). 553 tests pass / 5 skipped (was 540/5; +13); `yarn lint` clean (0/0); `yarn build` green. Files: `src/components/ConfirmDialog.tsx`, `src/components/ProtectedRoute.tsx`, `src/components/LoadingSpinner.tsx` (deleted), `src/app/globals.css`, `src/__tests__/confirmDialog.test.tsx` (new), `src/__tests__/protectedRoute.test.tsx` (new), `TODOS.md`. Branch: `claude/vigilant-cannon-UItv2`. Deferred (still on the 2026-05-18 list, untouched here): `AddMemoryModal`, `AddEventModal`, `AddFamilyModal`, `AddMemberModal`, `ImportGedcomModal`, `Modal`, `error.tsx`, `global-error.tsx`, `EmptyState`, `ResidencesEditor`, `FamilyList`, `UngeocodedSidebar`, `ProfileEditForm`, `WelcomeModal`, `AuthHero`, `PlacePopup`, `GenealogyTree`/`FamilyTreeView`, `AudioPlayer`, `auth/callback`, `admin/seed`, `families/[id]/page.tsx`.
