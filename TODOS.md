...
5. ~~**T-15** — `middleware.ts` for route-level auth.~~ Done 2026-05-21 — see Completed log. Shipped as `src/proxy.ts` (Next 16 file-convention rename) backed by a non-httpOnly `family_tree_auth` presence cookie written by `setStoredSession` in `src/lib/supabase.ts`.
...
- ~~**No edge-level auth.**~~ Resolved 2026-05-21. `src/proxy.ts` now gates browser navigation before the page shell renders, reading the `family_tree_auth` presence cookie written by `setStoredSession`. See T-15.
...
### ~~T-15. Add Next.js middleware for route protection~~ ✅ Done 2026-05-21

See Completed log. Shipped as `src/proxy.ts` (Next 16 renamed the `middleware` file convention to `proxy`; same edge runtime, same API). Gate redirects unauthenticated browser navigation to `/login?next=<original-path>`. The cookie it reads (`family_tree_auth`) is a presence indicator only — RLS remains the source of truth for data access.
...
- 2026-05-21 — **T-15 Edge-level auth gate via Next 16 `proxy.ts` + presence cookie.** Closed the long-standing "no edge-level auth" gap called out in the CLAUDE.md / TODOS executive summary. Until now, an unauthenticated user could load any page shell (`/family-tree`, `/profile/<id>`, `/memories`, etc.) and see loading skeletons / empty layouts before the client-side `ProtectedRoute` decided to redirect; RLS already refused the data, but the flash of UI was visible and every unauth load fired wasted Supabase requests on render. Fix has two halves: (1) `src/proxy.ts` (Next 16 renamed `middleware.ts` to `proxy.ts` — same edge runtime, same `next/server` API, just a different file convention and an `export function proxy`) runs ahead of route resolution and 307s any non-public path that lacks the auth cookie to `/login?next=<encoded-original-path>`. Public allowlist: `/login`, `/signup`, `/auth/callback`, `/forgot-password`, `/reset-password` (auth flows that must work logged-out) and `/api/*` in full (each API route manages its own auth contract — `verifyUser` for `/api/convert-image` and `/api/geocode`, `x-cron-secret` for `/api/notifications/digest`, `x-webhook-secret` for `/api/webhooks/*`, token-based for `/api/notifications/unsubscribe`, and `NODE_ENV` gate for `/api/seed`). The `config.matcher` excludes `_next/`, `favicon.ico`, and any path with a file extension so the proxy never runs against bundle chunks or static assets. (2) `src/lib/supabase.ts` now writes a non-httpOnly `family_tree_auth=1` presence cookie inside `setStoredSession` (alongside the existing localStorage write) and clears it with `Max-Age=0` on sign-out — including network-error sign-outs, since stranding a user with the cookie set but localStorage cleared would make the page shell render emptily. The cookie uses `Path=/`, `SameSite=Lax`, and `Secure` only when `location.protocol === "https:"` (omitting `Secure` on `http://localhost` so the gate doesn't silently break in local dev). Cookie name lives in `src/config/constants.ts` as `AUTH_INDICATOR_COOKIE` so the proxy and the auth client share one source of truth. The cookie is a *presence indicator only* — RLS remains the source of truth for data access; a forged cookie at most lets the empty page shell render, and the database still refuses the row queries. A small migration block at the bottom of `src/lib/supabase.ts` self-heals legacy installs at module load: if localStorage already holds a non-expired session but `document.cookie` doesn't have the indicator (because the user signed in before this deploy), it writes the cookie; if the stored token is expired, it clears the cookie so the user doesn't end up with a stale "looks signed in" indicator. Without this migration, every previously signed-in family member would have to re-authenticate once on next visit. Smoke-tested via `yarn dev` + curl: unauth `/family-tree` → `307` to `/login?next=%2Ffamily-tree`, unauth `/` → `307` with `next=%2F`, unauth `/profile/abc?tab=x` → `307` with properly-encoded `next=%2Fprofile%2Fabc%3Ftab%3Dx`, unauth `/api/webhooks/new-user` → passes through to the route, `/favicon.ico` → bypasses the proxy entirely (matcher exclusion), `/login` / `/signup` / `/reset-password` → all pass through to the page. Coverage: 23 new cases in `src/__tests__/proxy.test.ts` (public-path passthroughs, redirect status, `next=` preservation including a query-string round-trip, cookie-present passthrough, stale-cookie-value rejection, matcher shape pins) and 7 new cases in `src/__tests__/supabaseSessionCookie.test.ts` (cookie write on sign-in, clear on sign-out including network-error sign-out, legacy session migration on module load with valid + expired tokens, no-session module-load passthrough, cookie-attribute pin asserting `Path=/`, `SameSite=Lax`, and the absence of `Secure` under jsdom's `http://localhost`); plus 1 case in `configConstants.test.ts` pinning the cookie name to RFC-safe token characters. 571 tests pass / 5 skipped (was 540 / 5; +31); `yarn lint` clean (0 / 0); `yarn build` green and now prints `ƒ Proxy (Middleware)` in the route table; the previous `The "middleware" file convention is deprecated. Please use "proxy" instead.` dev-server warning is gone. Files: `src/proxy.ts` (new), `src/lib/supabase.ts`, `src/config/constants.ts`, `src/__tests__/proxy.test.ts` (new), `src/__tests__/supabaseSessionCookie.test.ts` (new), `src/__tests__/configConstants.test.ts`, `TODOS.md`. **Deferred follow-ups:** (a) login page does not yet read `?next=` and redirect back after sign-in — the proxy already encodes it on every redirect, so this is a future +30min polish; (b) the now-redundant client-side `ProtectedRoute` wrapper is left in place as belt-and-suspenders for race conditions (token expired mid-session, sign-out from another tab); collapsing it is a separate cleanup that should follow once we're confident the proxy gate is sufficient.
