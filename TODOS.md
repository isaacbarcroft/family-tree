...
### ~~T-14. Webhook new-user route: duplicate PostgREST filter param~~ ✅ Done 2026-05-15
...
- 2026-05-15 — **T-14 Webhook new-user route: collapse duplicate `email` filter into a single `and()` clause.** `src/app/api/webhooks/new-user/route.ts` previously built its recipient query by calling `URLSearchParams.append("email", ...)` twice — once for `not.is.null` and once for `neq.${newUserEmail}` — and relied on PostgREST's implicit AND-of-duplicate-keys to combine them. Replaced both with a single explicit `and=(email.not.is.null,email.neq."<escaped>")` parameter, and routed `newUserEmail` through `escapePgrstString` + surrounding `"…"` so reserved PostgREST characters (`,`, `()`, `\`, `"`) in user metadata can never break out of the literal. No behavior change for normal emails — PostgREST still ANDs both predicates — but the join is now stated, not inferred. Tests: added 2 new cases in `src/__tests__/webhookNewUser.test.ts` — (1) `URL.searchParams` round-trip pins the exact shape (`select=email`, single `and=(email.not.is.null,email.neq."new@example.com")`, no `email=` key) so the legacy duplicate-param shape regresses if reintroduced; (2) a quote-and-backslash email asserts the escape contract (`"weird\"\\@example.com"` inside the `and()` clause). 529 tests pass / 5 skipped (was 527/5; +2); `yarn lint` clean (0/0); `yarn build` green. Files: `src/app/api/webhooks/new-user/route.ts`, `src/__tests__/webhookNewUser.test.ts`, `TODOS.md`.
