...
**In priority order (refreshed 2026-05-20):**
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).** Would also upgrade the tree to W3C tree-widget semantics (`role="tree"` / `role="treeitem"` instead of `role="group"` / `role="button"`), since the tree role expects arrow-key roving tabindex.~~ Done 2026-05-20.
...
- 2026-05-20 — **1.6.a Arrow-key navigation on the D3 tree + W3C tree-widget semantics.** Upgraded `src/components/GenealogyTree.tsx` from `role="group"` + per-node `role="button"` to the WAI-ARIA tree pattern: the wrapping `<g>` now carries `role="tree"` with an updated `aria-label` that names the visualization and explains arrow-key + Enter / Space navigation, and every interactive person group in `src/components/TreeNode.tsx` switches from `role="button"` to `role="treeitem"` with `aria-level`, `aria-setsize`, `aria-posinset`, and `aria-expanded="true"` (only when the treeitem has children). Roving tabindex: a new `activeId` `useState` in `GenealogyTree` tracks the currently focusable treeitem, the active treeitem renders `tabIndex={0}`, every other treeitem renders `tabIndex={-1}`, and `onFocus` on each treeitem updates `activeId` so Tab and click both park the rover at the right place. A new `handleTreeKeyDown` on the tree `<g>` handles ArrowDown / ArrowUp (DFS-flat next / prev), ArrowRight (first child), ArrowLeft (parent), Home (first treeitem), and End (last treeitem); the matching DOM element is focused via `gRef.current.querySelector('[data-treeitem-id="..."]')` (every interactive group now also emits `data-treeitem-id`) with `CSS.escape` for safety. `preventDefault` fires on every consumed navigation key so the page does not scroll; other keys (e.g., letters, Tab) pass through. New pure helper `src/utils/treeNavigation.ts` exports `buildTreeitemIndex(layout)` (DFS walk that skips the synthetic family-root label, emits one treeitem per single-person LayoutNode and two per couple, computes level / setSize / posInSet / parent / childIds / prevId / nextId / hasChildren, and attributes a couple's children to the left half so ArrowLeft from a child returns to a single deterministic parent) and `nextTreeitemId(index, activeId, key)` (pure key → next-id resolver, exhaustive over ArrowDown / ArrowUp / ArrowRight / ArrowLeft / Home / End, returns null at the ends so no wrap). Tree-state reset on `treeData` change is implemented with the same render-time `useState` prev-tracking pattern the codebase already uses in `MemoryImage` / `ProfileAvatar` (the React 19 `react-hooks/refs` rule rejects accessing `ref.current` during render, so the pattern uses `useState` for the prev-tracker). Coverage: 22 new Vitest cases in `src/__tests__/treeNavigation.test.ts` (10 cover `buildTreeitemIndex`: synthetic-root skip, couple-half ARIA siblinghood, child attribution to the left half, 1-based level, prev / next chain with null ends, hasChildren flag, empty tree, multi-root setSize / posInSet; 12 cover `nextTreeitemId`: ArrowDown / ArrowUp through DFS, end-of-list null, ArrowRight to first child, ArrowRight on a leaf returns null, ArrowRight on the right half of a couple returns null, ArrowLeft to parent, top-level ArrowLeft returns null, Home / End always jump, ghost id returns null, Home / End still work when activeId is null), 9 new cases in `src/__tests__/genealogyTreeNavigation.test.tsx` (initial roving tabindex on first treeitem, ArrowDown advances the rover, ArrowLeft from a child returns to the left half of the parent couple, ArrowRight from a parent moves to the first child, Home / End jumps, no-wrap pin at the end of the list, navigation keys `preventDefault` while other keys pass through, focusing a treeitem updates the rover), and 5 updates to `src/__tests__/genealogyTreeA11y.test.tsx` (role swap from `group` → `tree`, label now mentions arrow keys, treeitem-children focusability assertion, plus a new "exactly one tabindex=0" rover invariant). The existing `src/__tests__/treeNode.test.tsx` was updated to thread the new `treeitemIndex` / `activeId` / `onTreeitemFocus` props through every render call; 3 new cases pin the role swap from `button` → `treeitem`, the `aria-expanded="true"` on a parent treeitem, the lack of `aria-expanded` on a leaf, and the `onTreeitemFocus` callback firing on `onFocus`. 571 tests pass / 5 skipped (was 535 / 5; +36); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/utils/treeNavigation.ts` (new), `src/components/GenealogyTree.tsx`, `src/components/TreeNode.tsx`, `src/__tests__/treeNavigation.test.ts` (new), `src/__tests__/genealogyTreeNavigation.test.tsx` (new), `src/__tests__/genealogyTreeA11y.test.tsx`, `src/__tests__/treeNode.test.tsx`, `TODOS.md`. Branch: `claude/keen-newton-tH6cY`.
