...
**In priority order (refreshed 2026-05-30):**
...
- Accessibility is largely in place (1.6.a / 1.6.c shipped); the remaining 1.6 sub-items (1.6.b inner-content landmarks, 1.6.d axe DevTools pass) are ~2h total
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).**~~ Done 2026-05-30 on `claude/keen-newton-oLAWl`. See Completed log.
...
**Effort:** ~2h for the remaining sub-items (1.6.b inner-content landmarks + 1.6.d axe DevTools pass).
...
- 2026-05-30 — **1.6.a Arrow-key navigation on the D3 tree (W3C tree-widget semantics).** Upgraded the genealogy tree from the prior `role="group"` wrapper + `role="button"` interactive `<g>` elements to a full WAI-ARIA tree widget with roving tabindex. The outer `<g>` in `src/components/GenealogyTree.tsx` now carries `role="tree"` and an updated `aria-label` that names every supported gesture ("Press Tab to enter, arrow keys to move between people, Home or End to jump to the first or last, and Enter or Space to open a profile."). Each interactive person `<g>` in `src/components/TreeNode.tsx` becomes a `role="treeitem"` with `aria-level`, `aria-posinset`, `aria-setsize`, `aria-selected`, plus a `data-treeitem-id` attribute used by the parent's focus-move effect. Roving tabindex contract: exactly one treeitem holds `tabIndex={0}` at any moment (initially the first treeitem in DFS pre-order); all other treeitems hold `tabIndex={-1}` so the SVG group occupies one Tab stop on the page. New `buildTreeNavMap` helper in `src/utils/treeLayout.ts` derives, for each treeitem id, its `level`, `parentId`, `firstChildId`, `prevSiblingId`, `nextSiblingId`, and `posInSet` / `setSize`. The "sibling group" definition is the key UX call: both halves of the same couple AND the treeitems of all sibling layout nodes share one flat sibling group at the same level — so ArrowRight from Alice goes to Bob (her right-half spouse) and from Bob continues to the next top-level node (Eve), matching the visual reading order. Canonical parent is the LEFT half of the parent couple (deterministic for ArrowUp; user can press ArrowRight to reach the right half). `levelOffset` distinguishes the synthetic family-root wrapper (children become level 1) from the single-root collapse in `buildHierarchy` (root itself is level 1). Arrow handler on the `<g role="tree">` parent: ArrowLeft/Right → prev/next sibling, ArrowUp → parent, ArrowDown → first child, Home → first treeitem, End → last treeitem in pre-order; every handled key fires `preventDefault` + `stopPropagation` so the surrounding scroll container does not pan. After `setActiveId`, a one-shot effect imperatively focuses the new treeitem via `data-treeitem-id` querySelector so screen readers and keyboard users follow. Click and focus on a treeitem also update the active id, so Tab/click/keyboard all converge on the same roving-tabindex state. Existing Enter / Space activation in `TreeNode` is preserved (and now also calls `stopPropagation` so the parent does not double-handle). Coverage: 11 new `treeNavMap.test.ts` cases (synthetic-root skip, couple-internal sibling order, cross-layout-node sibling flattening, parentId = first id of parent couple, parentId=null at top level, firstChildId = first id of first layout child, level numbering, DFS pre-order for Home/End, single-root collapse handled correctly, empty-root edge case), 12 new `genealogyTreeKeyboardNav.test.tsx` integration cases (initial active id, ArrowRight within couple, ArrowRight crossing couple boundary, no-wrap at last sibling, ArrowLeft, ArrowDown from parent / from right half of couple sharing same first child, ArrowUp returning to canonical parent, ArrowUp no-op at top, Home / End, preventDefault pin, Enter pass-through, focus event updating active id, single-tabIndex-0 invariant across a sequence of arrow presses). Existing `treeNode.test.tsx` (16 cases) updated to assert `role="treeitem"` instead of `role="button"` and to thread `navMap` / `activeId` / `onFocus` props through every render call; two new cases added (inactive treeitem has tabIndex=-1 + aria-selected=false; data-treeitem-id present on every interactive treeitem). `genealogyTreeA11y.test.tsx` (4 cases) re-pointed at `role="tree"` and now asserts the single-tabIndex-0 invariant on first render. 579 tests pass / 5 skipped (was 560/5; +19); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/utils/treeLayout.ts`, `src/components/TreeNode.tsx`, `src/components/GenealogyTree.tsx`, `src/__tests__/treeNavMap.test.ts` (new), `src/__tests__/genealogyTreeKeyboardNav.test.tsx` (new), `src/__tests__/treeNode.test.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `TODOS.md`. Branch: `claude/keen-newton-oLAWl`. **Deferred:** (a) typeahead search (typing letters to focus matching names) is in the WAI-ARIA tree pattern but is a separate UX scope — the NavBar global search already covers cross-tree lookup; (b) expand / collapse via Left / Right at branch nodes is not applicable here because the family tree does not collapse subtrees today.
