...
2. ~~**1.6.c Focus management after tree node click.**~~ Shipped 2026-05-24 — see Completed log. Implemented at the layout level so every `router.push(...)` navigation (not just from tree nodes) re-anchors focus on the new page's `<main>`.
...
5. **T-15** — `middleware.ts` for route-level auth. Defense-in-depth on top of RLS. **Note 2026-05-24:** the 1h estimate is wrong — the current Supabase session lives in `localStorage` (see `src/lib/supabase.ts:17`), not in cookies, so Next.js middleware can't read auth state at the edge. Implementing T-15 requires first migrating session storage to cookies (or adding a server-readable "session hint" cookie alongside localStorage). Re-scope before pulling.
...
- ~~**1.6.c Focus management after tree node click.**~~ Done 2026-05-24. Generalized beyond tree-click: a new `RouteFocusManager` client component watches `usePathname()` and moves focus to the `<main>` landmark after any client-side navigation (skipping the initial mount so the browser keeps owning first-load focus). See Completed log.
...
- 2026-05-24 — **1.6.c Focus management after client-side navigation.** The remaining ~1h sub-piece of the 1.6 a11y pass. Scoped at the layout level rather than just on tree-node clicks so every `router.push(...)` in the app benefits (NavBar search results, family-tree node activation, family detail page member links, profile-page family chips, timeline → profile, etc.). New `src/components/RouteFocusManager.tsx` is a render-nothing client component that watches `usePathname()`, tracks the previous pathname via a ref, skips the initial mount (browser handles first-load focus), and on each subsequent pathname change calls `main.focus({ preventScroll: true })` on the `#main-content` landmark. `preventScroll` keeps Next.js's normal scroll-to-top behavior in charge of the viewport rather than the focus call yanking it. `src/app/layout.tsx` now renders `<main id={MAIN_LANDMARK_ID} tabIndex={-1}>` (the `-1` makes the landmark programmatically focusable without inserting it into the natural tab order) plus the `<RouteFocusManager />` sibling. `MAIN_LANDMARK_ID = "main-content"` lives in `src/config/constants.ts` alongside the other shared layout constants (`PLACES_MAP_HEIGHT`, `GENEALOGY_TREE_HEIGHT`) so both consumers (layout + manager) reference the same source of truth. `src/app/globals.css` adds a scoped `main:focus, main:focus-visible { outline: none }` rule — the programmatic focus on a layout container would otherwise paint the global sage focus ring around the entire page content, which is visual noise (keyboard users still see focus move because the next Tab starts from inside `<main>`). Coverage: 7 new behavior cases in `src/__tests__/routeFocusManager.test.tsx` (no-op on initial mount, focus moves on pathname change, re-focuses on each distinct change, no-op on same-pathname re-render, no-op when landmark absent, `preventScroll: true` is forwarded, renders nothing); 3 new layout regression-pin cases in `src/__tests__/layoutMainLandmark.test.ts` (imports `MAIN_LANDMARK_ID`, renders `<main id=… tabIndex={-1}>`, mounts `<RouteFocusManager />`); 1 new constant pin in `configConstants.test.ts` (`MAIN_LANDMARK_ID` is a non-empty, CSS-id-safe string). 551 tests pass / 5 skipped (was 544/5; +7); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/components/RouteFocusManager.tsx` (new), `src/app/layout.tsx`, `src/app/globals.css`, `src/config/constants.ts`, `src/__tests__/routeFocusManager.test.tsx` (new), `src/__tests__/layoutMainLandmark.test.ts` (new), `src/__tests__/configConstants.test.ts`, `TODOS.md`. **Manual UI verification deferred:** the dev server fails to render in this environment due to a pre-existing Turbopack-only CSS parse error (`var()` with no argument in the generated post-Tailwind output around line 1436; reproduces on `main` unchanged, `yarn build` is unaffected). The change is purely behavioral (focus position after navigation) with no visual diff, so the unit tests cover the contract end-to-end. Real-device screen-reader verification (VoiceOver / NVDA announcing the new page on each navigation) belongs with the 1.6.d axe-DevTools pass.
