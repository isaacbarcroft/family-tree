...
`src/components/ProfileAvatar.tsx` already uses `next/image`. Memory thumbnails on `/memories` and `/profile/[id]` now route through `src/components/ui/PhotoFrame.tsx` (post-UI-refresh, PR #28) which still renders a plain `<img>`. Switch `PhotoFrame` to `next/image` with explicit `sizes`. Generate thumbnail variants on upload (sharp in an edge function or server action). The legacy `MemoryImage` component referenced in earlier audits was removed on 2026-05-16 — it was dead code after the UI refresh.
**Effort:** 3h
...
- 2026-05-16 — **Tech debt: remove dead `MemoryImage` component.** `src/components/MemoryImage.tsx` (the original HEIC-aware thumbnail wrapper) had zero callers in `src/app/` or `src/components/` outside its own test file; the per-tile renders on `/memories` and `/profile/[id]` were migrated to `src/components/ui/PhotoFrame.tsx` during the UI refresh (PR #28, `c9a56f8`), and `ProfileAvatar.tsx` already routes through `next/image` for its own path. Deleted `src/components/MemoryImage.tsx` (44 lines) and `src/__tests__/memoryImage.test.tsx` (66 lines). Five components carried comments citing `MemoryImage` as the canonical example of the React 19 "reset state during render via `prev*` tracking" pattern; rewrote each one to cite the current implementation sites instead (`ProfileAvatar` / `PhotoFrame`): `src/components/ResidencesEditor.tsx`, `src/components/AddMemberModal.tsx`, `src/components/ProfileAvatar.tsx`, `src/components/NavBar.tsx`, `src/components/ui/PhotoFrame.tsx`, `src/components/ui/Avatar.tsx`. Updated the still-pending T-3 entry so it no longer claims `MemoryImage` is one of the migration targets. 522 tests pass / 5 skipped (was 527/5; net −5 from the removed `memoryImage.test.tsx` cases — no other behavior changed); `yarn lint` clean (0/0); `yarn build` green. Files: `src/components/MemoryImage.tsx` (deleted), `src/__tests__/memoryImage.test.tsx` (deleted), `src/components/ResidencesEditor.tsx`, `src/components/AddMemberModal.tsx`, `src/components/ProfileAvatar.tsx`, `src/components/NavBar.tsx`, `src/components/ui/PhotoFrame.tsx`, `src/components/ui/Avatar.tsx`, `TODOS.md`. Branch: `claude/vigilant-cannon-f6vlu`.
