...
3. **1.6.d Run [axe DevTools](https://www.deque.com/axe/devtools/)** and fix all critical / serious issues. Best done after 1.6.b and 1.6.c so the tree and landmark changes are reflected in the report.
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).**~~ Done 2026-05-21 (PR #69). `src/components/GenealogyTree.tsx` now upgrades the SVG interaction model to `role="tree"` with roving `tabIndex`, arrow-key movement between visible people, and `Home` / `End` shortcuts. `src/components/TreeNode.tsx` accepts per-person treeitem props so the existing Enter / Space activation still opens the right profile for single-person nodes and each half of a couple. Coverage: `src/__tests__/genealogyTreeA11y.test.tsx` now verifies the tree role, treeitems, roving focus, vertical parent/child movement, horizontal spouse movement, and Enter navigation.
...
- 2026-05-21 — **1.6.a Arrow-key navigation on the D3 tree.** The genealogy SVG now behaves like a real tree widget instead of a labelled group with plain Tab stops. `src/components/GenealogyTree.tsx` upgrades the content wrapper to `role="tree"`, derives one focusable item per visible person from the existing layout data, and applies roving `tabIndex` plus `aria-level` / `aria-posinset` / `aria-setsize`. Arrow keys move through the visible tree by row and nearest vertical match, `Home` jumps to the first person, `End` jumps to the last person, and Enter / Space still open the selected profile. `src/components/TreeNode.tsx` now accepts treeitem wiring from the parent so single-person nodes and each spouse half can share the same keyboard contract without losing the existing click handlers or screen-reader labels. Coverage: `src/__tests__/genealogyTreeA11y.test.tsx` was expanded from 4 screen-reader assertions to 7 tree-widget assertions covering the `tree` label, decorative `<defs>` / edge hiding, roving focus, ArrowDown parent-to-child movement, ArrowRight spouse-to-spouse movement, and Enter navigation. Validation on this branch: `yarn lint`, `yarn test`, and `yarn build` all passed; the build still reports the pre-existing CSS optimizer warning from `src/app/globals.css` around the `memory-reaction-button` hover note. Files: `src/components/GenealogyTree.tsx`, `src/components/TreeNode.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `TODOS.md`. PR #69.
