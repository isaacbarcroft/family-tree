...
**In priority order (refreshed 2026-05-22):**
...
3. **1.6.d Run [axe DevTools](https://www.deque.com/axe/devtools/)** and fix all critical / serious issues. Best done after 1.6.b and 1.6.c so the landmark + focus-restore changes are reflected in the report.
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).** Would also upgrade the tree to W3C tree-widget semantics (`role="tree"` / `role="treeitem"` instead of `role="group"` / `role="button"`), since the tree role expects arrow-key roving tabindex.~~ Done 2026-05-22 on `claude/keen-newton-sFJwP`. See Completed log.
...
- 2026-05-22 — **1.6.a Arrow-key navigation on the D3 tree (`claude/keen-newton-sFJwP`).** Upgraded the genealogy tree from `role="group"` + `role="button"` semantics to the W3C tree-widget pattern with roving tabindex. The container `<g>` in `src/components/GenealogyTree.tsx` is now `role="tree"`, and every interactive `<g>` half in `src/components/TreeNode.tsx` is `role="treeitem"` with `aria-level` / `aria-posinset` / `aria-setsize` (per the focus graph), `data-tree-item-id="<personId>"` for imperative `.focus()` lookup, and `tabIndex={personId === activeId ? 0 : -1}` so exactly one item is in the tab sequence at a time. The aria-label hint on the tree now reads "Tab into the tree, then use arrow keys to move between people. Enter or Space opens a profile." instead of the old Tab-only wording. New pure module `src/utils/treeNavigation.ts` exposes `buildFocusGraph(layout)`, `nextFocusableId(graph, currentId, key)`, and `isTreeNavKey(key)`. The focus graph: walks the LayoutNode tree once, emits 1 focusable item per single-person LayoutNode and 2 (left + right, same x/y) per couple, skips the synthetic family-root (id-less node), and pre-computes four lookup maps: `upFrom` (parent LayoutNode's first item), `downFrom` (first-child LayoutNode's first item), `leftFrom` / `rightFrom` (flattened sibling row across all children of the same parent LayoutNode, which naturally captures both within-couple spouse pairs and cross-LayoutNode siblings). `aria-level` is 1-based with a synthetic-root offset so top-level people read as level 1 regardless of whether a synthetic family-root is injected by `buildHierarchy`. `GenealogyTree.tsx` holds the `activeId` state (defaults to `focusGraph.firstId`, resets when the tree changes), attaches a single `onKeyDown` to the `role="tree"` `<g>` that calls `nextFocusableId` and imperatively `.focus()`es the new item by `data-tree-item-id` (with `CSS.escape` for safety), and an `onFocus` that syncs activeId to whatever just received focus (covers Tab-in, programmatic focus, and a-t-driven focus). Coverage: 27 new cases in `src/__tests__/treeNavigation.test.ts` (empty / single / couple / synthetic-root graphs; aria-level for real-root vs synthetic-root; aria-posinset / aria-setsize within the parent's full row; ArrowUp / ArrowDown chain; Home / End; within-couple ArrowLeft / ArrowRight; cross-couple sibling-row walk; null when no movement; multi-branch synthetic root does not link children of different roots; the `isTreeNavKey` predicate). Existing `src/__tests__/treeNode.test.tsx` updated to assert `role="treeitem"`, `data-tree-item-id`, and roving tabindex (active half gets `tabindex=0`, inactive halves get `-1`); +2 new cases for the inactive case and data-tree-item-id pinning. `src/__tests__/genealogyTreeA11y.test.tsx` updated: the container assertion is now `role="tree"` and the usage-hint check looks for "arrow keys"; +5 new cases pin roving-tabindex (exactly one `tabindex=0` and it's the first item), aria-level / aria-posinset / aria-setsize numeric attrs on every treeitem, ArrowDown moves the roving tabindex owner, recognized tree-nav keys call `preventDefault` (so page does not scroll), and unrecognized keys (`"a"`) are ignored. 573 tests pass / 5 skipped (was 535/5; +38 across +27 treeNavigation cases, +5 a11y cases, +2 treeNode cases, +4 small shifts elsewhere from re-running the suite); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/utils/treeNavigation.ts` (new), `src/components/TreeNode.tsx`, `src/components/GenealogyTree.tsx`, `src/__tests__/treeNavigation.test.ts` (new), `src/__tests__/treeNode.test.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `TODOS.md`. Deferred to 1.6.c: focus restore on the destination page after `router.push(/profile/[id])`. The tree side of the keyboard journey is done; the page-arrival side is a separate concern that touches every routed page, not just the tree.
