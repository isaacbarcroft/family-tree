...
- 2026-05-29 — **Bug: leap-day (Feb 29) birthdays/anniversaries shifted to Mar 1 in non-leap years.** Audit-driven, not a tracked TODO. Three recurring-date computations built the in-year occurrence with `new Date(year, monthIndex, day)`, which overflows an out-of-range day into the next month — so a Feb 29 birthday became **Mar 1** in every non-leap year. Concrete impact: (a) the home-page "upcoming birthdays" widget (`getNextBirthday` / `getAge` in `src/utils/dates.ts`) displayed the wrong date and could be off-by-one on the "turning N" count for leap-day people; (b) the birthday-reminder + "on this day" digest emails (`findOccurrenceInWindow` in `src/utils/digest.ts`) computed Mar 1, which fell outside a digest window ending Feb 28 and **silently dropped the leaper's birthday** for that cycle. Fix: new pure helper `recurringDateInYear(year, monthIndex, day)` in `src/utils/dates.ts` clamps an overflowed day to the intended month's last day (`new Date(year, monthIndex + 1, 0)`), so Feb 29 → Feb 28 in non-leap years and stays Feb 29 in leap years — the common convention for observing leap-day birthdays. Routed all three call sites through it: `getNextBirthday`, `getAge` (also simplified from the hand-rolled month/day comparison to a single `today < birthdayThisYear` check — still no `else`), and `digest.findOccurrenceInWindow`. No schema, markup, or styling change — purely corrected date values, so no screenshot applies. Coverage: +8 cases in `src/__tests__/dates.test.ts` (getNextBirthday clamps Feb 29→Feb 28 non-leap / keeps Feb 29 leap / rolls a passed Feb 29 into a clamped next year; getAge treats Feb 28 as the birthday in a non-leap year and stays pre-birthday on Feb 27; plus 3 direct `recurringDateInYear` contract cases) and +1 case in `src/__tests__/digestBuilder.test.ts` (a Feb 29 birthday now lands on Feb 28 inside a (Feb 25, Feb 28] window with age 34, instead of being skipped). 559 tests pass / 5 skipped (+9); `yarn lint` clean (0/0); `yarn build` green. Files: `src/utils/dates.ts`, `src/utils/digest.ts`, `src/__tests__/dates.test.ts`, `src/__tests__/digestBuilder.test.ts`, `TODOS.md`. **Assumption flagged for review:** chose the Feb-28 clamp (most common convention, and what most date libraries do); if you'd rather observe leap-day birthdays on Mar 1, the single `recurringDateInYear` helper is the one place to flip it. Branch: `claude/vigilant-cannon-iCsyl`.
