...
2. ~~**1.6.c Focus management after tree node click.**~~ Done 2026-05-20. `src/hooks/useFocusOnIdChange.ts` plus profile-page wiring move focus to the first profile `<h1>` (with `tabIndex={-1}` and `{ preventScroll: true }`) the first time a `person.id` resolves and on every subsequent navigation to a different person.
...
- ~~**1.6.c Focus management after tree node click.**~~ Done 2026-05-20. New hook `src/hooks/useFocusOnIdChange.ts` moves focus to a ref the first time it's mounted with a non-empty id and on every subsequent id change, using `{ preventScroll: true }` so mouse users don't see scroll jank. Wired into `src/app/profile/[id]/page.tsx`: the parent owns `headingRef` and the hook (so the tracking ref survives the edit / exit-edit toggle that unmounts `Hero`), and `Hero` attaches `ref={headingRef}` + `tabIndex={-1}` to the first profile `<h1>`. The global `*:focus-visible` rule in `globals.css:135` still paints the focus ring for keyboard users; mouse users get no ring per the browser's `:focus-visible` heuristic. Coverage: 5 new Vitest cases in `src/__tests__/useFocusOnIdChange.test.tsx` (focus on first mount with id, skip when id is null, refocus on id change, no refocus on same-id re-render, calls `focus({ preventScroll: true })`).
...
- 2026-05-20 — **1.6.c Focus management after tree node click (`claude/vigilant-cannon-Ywtam`).** New hook `src/hooks/useFocusOnIdChange.ts` moves keyboard focus to a `RefObject<HTMLElement | null>` the first time it's mounted with a non-empty id and again on every subsequent change to a different non-empty id; same-id re-renders are skipped via an internal `lastFocusedIdRef` so the hook does not yank focus on unrelated re-renders. `focus({ preventScroll: true })` is used throughout so mouse users do not see a scroll jump on initial load. Wired into `src/app/profile/[id]/page.tsx`: the parent (`ProfileContent`) owns the `headingRef` and calls the hook with `person?.id`, so the tracking ref survives the edit / exit-edit toggle that unmounts `Hero`; `Hero` accepts `headingRef?: React.RefObject<HTMLHeadingElement | null>` and attaches it (plus `tabIndex={-1}`) to the first profile `<h1>` (firstName + middleName). The previously-existing `*:focus-visible` rule in `globals.css:135` paints the focus outline for keyboard users; mouse users see no ring per the browser's `:focus-visible` heuristic, so the change is invisible to mouse navigation. Coverage: 5 new Vitest cases in `src/__tests__/useFocusOnIdChange.test.tsx` (focus on first mount with an id, no focus when id is null, refocus on id change, no refocus on same-id re-render, uses `{ preventScroll: true }`). 545 tests pass / 5 skipped (was 540/5; +5); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/hooks/useFocusOnIdChange.ts` (new), `src/__tests__/useFocusOnIdChange.test.tsx` (new), `src/app/profile/[id]/page.tsx`, `TODOS.md`.
