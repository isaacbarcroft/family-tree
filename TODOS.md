...
5. ~~**1.4** — guided story prompts.~~ Core shipped 2026-05-28 (PR #81). Remaining: 1.4.a AI follow-up prompts, 1.4.b structured `memories.promptId` link.
...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ — core done 2026-05-28 (PR #81)

See Completed log. Core shipped: `story_prompts` table (70 seeded prompts, 7 categories, allowlist-gated read-only RLS), `pickDailyPrompt` selector, and the home-page "A question for you today" widget with "Answer with text" / "Answer with voice" CTAs that open `AddMemoryModal` prefilled with the prompt. **Open follow-ups:**

- **1.4.a AI-generated follow-up prompts.** After a user answers, call the Claude API to suggest a tailored follow-up question. +4h. Reference: [StoryWorth](https://welcome.storyworth.com/) does this manually over email.
- **1.4.b Structured `memories.promptId` link.** The core sub-scope "tags" an answer by prefilling the memory title with the prompt question (no `memories` schema change). A nullable `promptId` FK on `memories` would make answers queryable by prompt and enable a "prompts you've answered" view. +2h. Needs a `memories` migration + `Memory` model field threaded through `addMemory` and the widget.
- **1.4.c Admin prompt curation UI.** Prompts are managed via seed/SQL today. A small admin page to add/retire prompts is a deferred polish item. +2h.
...
- 2026-05-28 — **1.4 Guided story prompts ("Ask Grandma" mode) — core sub-scope (PR #81).** Shipped the 4h core of 1.4; the +4h AI follow-ups split out as 1.4.a (plus 1.4.b structured link, 1.4.c admin UI). New migration `20260528_story_prompts.sql` (+ rollback) creates `public.story_prompts (id, prompt unique, category checked-enum, "createdAt")`, indexes `category`, enables RLS, and grants **only** `select` to `authenticated` (no insert/update/delete — prompts are reference data, seeded in-migration where the migration role bypasses RLS, curated via SQL). The `story_prompts_select_approved` policy gates reads on `public.is_approved_user()`, mirroring the `app_users` allowlist model. Seeded **70 prompts** across the seven categories (childhood, career, love, faith, travel, holidays, pets); `on conflict (prompt) do nothing` keeps re-runs idempotent and the migration introduces no destructive ops. New `src/models/StoryPrompt.ts` (model + `STORY_PROMPT_CATEGORIES` const + `StoryPromptCategory` type). New `src/utils/storyPrompt.ts`: pure `dayNumber(date)` (local Y/M/D → whole days since epoch, stable across a day) and `pickDailyPrompt(prompts, date)` which sorts by id then advances one prompt per calendar day (deterministic regardless of fetch order, never out of bounds, does not mutate input), plus `categoryLabel`. New `listStoryPrompts()` in `src/lib/db.ts` (orders by `createdAt`). New `src/components/StoryPromptWidget.tsx` (client): fetches prompts, picks the daily one, renders "A question for you today" + category label + the prompt with **Answer with text** / **Answer with voice** CTAs; renders nothing when the table is empty. Wired into the authenticated home page (`src/app/page.tsx`) above the hero spread. `AddMemoryModal` gained backward-compatible optional props `initialTitle`, `initialDescription`, `autoStartRecording` (the voice CTA auto-starts recording; the effect captures the mount-time `startRecording` in a ref to stay dependency-clean). "Tagging" in this sub-scope = prefilling the memory title with the prompt question (no `memories` schema change; structured `promptId` deferred to 1.4.b). Coverage: 21 new cases — migration regex suite (`storyPromptsMigration.test.ts`: table shape, unique prompt, category check, grant-select-only + no insert/update/delete, allowlist SELECT policy, seed ≥50 + every category present, category index, no destructive ops, rollback), `storyPrompt.test.ts` (dayNumber stability/increment, pickDailyPrompt empty/single/stable/order-independent/rotation/bounds/no-mutation, categoryLabel), `storyPromptsDb.test.ts` (URL + order + error propagation), `storyPromptWidget.test.tsx` (render + empty + text/voice modal-open with prefilled title and autostart flag), and two AddMemoryModal cases (prefill + auto-record). 581 tests pass / 5 skipped (was 560/5; +21); `yarn lint` clean (0/0); `yarn build` green. Files: `supabase/migrations/20260528_story_prompts.sql` (new), `supabase/migrations/20260528_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/utils/storyPrompt.ts` (new), `src/components/StoryPromptWidget.tsx` (new), `src/lib/db.ts`, `src/components/AddMemoryModal.tsx`, `src/app/page.tsx`, `SUPABASE_SETUP.md`, `TODOS.md`, plus 4 new test files and `src/__tests__/addMemoryModalAudio.test.tsx`. Branch: `claude/keen-newton-dEibJ`. **Not applied to any Supabase project from this session** (no DB credentials available); reviewed-by-test only — apply on a Supabase branch first per `SUPABASE_SETUP.md`.
