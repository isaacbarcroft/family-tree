...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ Done 2026-05-06
...
**Outcome:** Migration `20260506_story_prompts.sql` (+ rollback) adds `public.story_prompts (id, text, category, isActive, createdAt)`, with a category check constrained to the seven canonical categories (childhood, career, love, faith, travel, holidays, pets), a `length(btrim(text)) between 1 and 500` body check, and partial / regular indexes on `category` and `isActive`. Same migration adds a nullable `"storyPromptId" uuid` to `public.memories` with `references story_prompts(id) on delete set null` so memory rows survive prompt deletion, plus a partial index `where "storyPromptId" is not null`. RLS uses the existing allowlist helpers: SELECT for any approved user, INSERT/UPDATE/DELETE locked to admins (the catalog is curated, no user-typed prompts in the MVP). The migration seeds 60 prompts in one idempotent `insert ... select ... where not exists` block (re-running the migration does not duplicate rows). New `StoryPrompt` model + `STORY_PROMPT_CATEGORIES` constant. New `listStoryPrompts` and `listAnsweredStoryPromptIdsForUser` in `src/lib/db.ts`. New pure `src/utils/storyPromptPicker.ts` exports `pickStoryPrompt(prompts, answeredIds, userId, dateString, skipOffset)` (deterministic per (userId, date), filters out inactive + already-answered prompts, supports skip-rotation that wraps) and `todayDateString(now)`. New `StoryPromptWidget` component renders a "A question for you today" card with the prompt text plus three actions: "Answer with text" (opens `AddMemoryModal` pre-tagged with the prompt), "Answer with voice" (same modal, sets `startWithRecording={true}` so the `MediaRecorder` flow auto-starts on mount via a once-per-mount ref guard), and "Show me a different prompt" (increments `skipOffset`); shows a skeleton loader while data loads, hides the section if loading fails, and shows a "you have answered every prompt" empty state when nothing is left. Wired into `/` (signed-in home view) right under the secondary-actions row. `AddMemoryModal` gains two new optional props: `storyPrompt: { id, text }` (pre-fills the title with the prompt and renders a "Answering today's prompt" banner) and `startWithRecording` (auto-starts recording on mount, gated by a ref so it fires exactly once even on re-render); `addMemory` now persists `storyPromptId` so the same prompt is excluded from that user's future picker calls. Coverage: 12 migration regex assertions (table shape + checks + indexes + RLS gates including `is_admin_user` for write paths + FK on memories + seed-with-NOT-EXISTS + per-category seed presence + no-destructive guarantee + rollback contents), 6 db cases (URL filter for `isActive=is.true` + order, propagated errors, `createdBy=eq` + `deletedAt=is.null` + `select=storyPromptId` for the answered-id helper, de-duping + null-stripping, error propagation), 12 picker cases (empty list, all answered, inactive-only, deterministic for same (user, date), date rotation across 30 days, exclude answered, skipOffset rotation, skipOffset wrap-around, different users see different prompts), 8 widget cases (skeleton state, both action buttons render, "Answer with text" passes `startWithRecording=false` and forwards `preTaggedPersonId`, "Answer with voice" passes `startWithRecording=true`, "Show me a different prompt" rotates the visible text, every-answered empty state hides the buttons, modal close + answered-refetch on `onCreated`, fail-quiet when prompt loading throws). 503 tests pass / 5 skipped; lint clean (0 errors / 0 warnings); `yarn build` green; `tsc --noEmit` clean.
**Follow-ups (deferred):**

- 1.4.a AI-generated follow-up prompts based on the user's answer (Claude API call). Estimated +4h. The original ticket flagged this as the optional split. Hooks into `addMemory` post-create and writes a follow-up prompt back into `story_prompts` (or a per-user override table) so the next day's pick can build on the previous day's answer.
- 1.4.b Per-category filtering / "tonight's category" UI. Today the picker is global. A small chip row would let users self-select "give me a holidays prompt." Estimated +1h.
- 1.4.c Self-service "request a prompt" admin UI. Today new prompts require a service-role insert (one-liner SQL documented in `SUPABASE_SETUP.md`). An admin page that uses the existing admin-gated INSERT policy is a natural follow-up. Estimated +2h.
...
- 2026-05-06 — **1.4 Guided story prompts ("Ask Grandma" mode).** Migration `20260506_story_prompts.sql` (+ rollback) creates `public.story_prompts (id, text, category, isActive, createdAt)` with a category check constrained to seven canonical categories (childhood, career, love, faith, travel, holidays, pets), a body length constraint, and per-column indexes; adds a nullable `"storyPromptId"` to `public.memories` with `on delete set null` so memory rows survive prompt deletion; RLS gates SELECT on `is_approved_user()` and locks all three write verbs to `is_admin_user()` (the catalog is curated, no user-typed prompts in the MVP). Seeds 60 prompts spread across the seven categories via a NOT EXISTS guard so re-running is idempotent. New pure `pickStoryPrompt(prompts, answeredIds, userId, dateString, skipOffset)` in `src/utils/storyPromptPicker.ts` deterministically chooses today's prompt by hashing `(userId, dateString)`, filters out inactive + already-answered prompts, and supports skip-rotation that wraps. New `StoryPromptWidget` renders a "A question for you today" card on the signed-in home view with three actions: "Answer with text" (opens `AddMemoryModal` pre-tagged with the prompt), "Answer with voice" (same modal but auto-starts the existing `MediaRecorder` flow on mount via a once-per-mount ref guard), and "Show me a different prompt" (rotates locally). Skeleton state on first load, fail-quiet when prompts cannot be loaded, "every prompt answered" empty state when the user has caught up. `AddMemoryModal` gains two new optional props (`storyPrompt: { id, text }`, `startWithRecording: boolean`), pre-fills the title with the prompt, renders a small "Answering today's prompt" banner, and persists `storyPromptId` on the new memory so future picks exclude it. Coverage: 12 migration cases (`storyPromptsMigration.test.ts`), 6 db cases (`storyPromptsDb.test.ts`), 12 picker cases (`storyPromptPicker.test.ts`), 8 widget cases (`storyPromptWidget.test.tsx`). 503 tests pass / 5 skipped; lint clean (0 errors / 0 warnings); `yarn build` green; `tsc --noEmit` clean. `SUPABASE_SETUP.md` lists the new migration and documents the admin-side insert/retire SQL one-liners. Files: `supabase/migrations/20260506_story_prompts.sql` (new), `supabase/migrations/20260506_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/models/Memory.ts`, `src/utils/storyPromptPicker.ts` (new), `src/lib/db.ts`, `src/components/StoryPromptWidget.tsx` (new), `src/components/AddMemoryModal.tsx`, `src/app/page.tsx`, `src/__tests__/storyPromptsMigration.test.ts` (new), `src/__tests__/storyPromptsDb.test.ts` (new), `src/__tests__/storyPromptPicker.test.ts` (new), `src/__tests__/storyPromptWidget.test.tsx` (new), `SUPABASE_SETUP.md`, `TODOS.md`. Deferred (call out in PR for review): 1.4.a AI follow-up prompts (the +4h optional split from the original ticket), 1.4.b per-category filtering / "tonight's category" chip row, 1.4.c self-service admin UI for adding new prompts (today new prompts require a service-role insert; the admin INSERT policy is in place, just needs UI).
