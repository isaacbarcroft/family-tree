...
**In priority order (refreshed 2026-05-24):**
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).** Would also upgrade the tree to W3C tree-widget semantics (`role="tree"` / `role="treeitem"` instead of `role="group"` / `role="button"`), since the tree role expects arrow-key roving tabindex.~~ Done 2026-05-24. See Completed log.
...
- 2026-05-24 — **1.6.a Arrow-key navigation on the D3 tree (W3C tree-widget semantics).** The previous 1.6 pass gave each interactive `<g>` `role="button"` + `tabIndex={0}`, so Tab cycled into every person sequentially, every focusable node held `tabIndex=0` (no roving), and arrow keys did nothing (so a 100-person tree took ~100 Tab presses to traverse). This branch upgrades to the W3C tree-widget pattern. (1) New pure `buildTreeNavigation(layout): { items, byId }` in `src/utils/treeLayout.ts` walks the laid-out tree depth-first pre-order and emits a `TreeItem` per person id with `parentId`, `firstChildId`, `prevInDomOrder`, `nextInDomOrder`, `level`, `posInSet`, `setSize`. For couples both halves emit (left then right) and share the same `firstChildId` (the couple's first descendant) so ArrowRight descends regardless of which spouse is focused; the children of a couple anchor to the left half via `parentId` so ArrowLeft from a child lands on the husband/wife at the canonical "left" of the parent box and ArrowRight then moves to the spouse. (2) `src/components/TreeNode.tsx` switches the three interactive `<g>` branches from `role="button"` to `role="treeitem"` with `aria-level` / `aria-posinset` / `aria-setsize` per item, the `tabIndex` is now driven by `focusedItemId === id ? 0 : -1` (roving), the `onKeyDown` handler still routes Enter / Space through `onNavigate` (with `preventDefault` on Space) but now also routes ArrowUp / ArrowDown / ArrowLeft / ArrowRight / Home / End through a new `onArrowKey(id, key)` callback (with `preventDefault` so the page does not scroll on the arrows either), and the synthetic family-root label is now marked `aria-hidden="true"` so a `role="tree"` container only exposes `role="treeitem"` children to assistive tech. (3) `src/components/GenealogyTree.tsx` switches the inner `<g>` from `role="group"` to `role="tree"` with the aria-label "Family tree. Use arrow keys to move between people. Press Enter or Space to open a profile.", owns the `focusedItemId` state (initialized to the first DOM-order item, kept in sync with tree-data swaps via the render-time `prev*` pattern so we stay clear of `react-hooks/set-state-in-effect`), holds a `Map<personId, SVGGElement>` of item refs via `registerItemRef`, and translates arrow keys into focus moves through a pure `resolveArrowTarget(navigation, currentId, key)` helper. After a roving update lands in the DOM, an effect gated on a `pendingFocusRef` (so click / programmatic focus does not loop) calls `.focus()` on the new treeitem so the visible focus ring follows the announced item. `onItemFocus` lifts click-driven focus changes back into state so the next arrow press picks up where the user clicked. Mapping mirrors the W3C tree pattern: ArrowUp / ArrowDown = previous / next visible treeitem in DOM order; ArrowRight = first child treeitem (or no-op at a leaf); ArrowLeft = parent treeitem (or no-op at root); Home / End = first / last treeitem. The existing `:focus-visible` stroke-change CSS in `globals.css:225` continues to paint the focus ring on whichever treeitem is currently focused. Coverage: (a) 10 new cases in `src/__tests__/treeNavigation.test.ts` pin the navigation map (DFS pre-order, parent / firstChild / sibling links, level / posInSet / setSize grouping by shared `(parent, level)`, single-root and empty-tree edge cases, top-level-children-of-synthetic-root sibling grouping); (b) 9 new cases in `src/__tests__/genealogyTreeKeyboard.test.tsx` integration-test the full keyboard flow (initial roving tabindex, ArrowDown/Up cycling through DOM order, ArrowRight descend, ArrowRight at a leaf is a no-op, ArrowLeft ascend through couple-left, ArrowLeft at root is a no-op, Home / End, click-driven focus updates roving tabindex); (c) the existing `src/__tests__/treeNode.test.tsx` was rewritten to feed the new `focusedItemId` / `onArrowKey` / `onItemFocus` / `registerItemRef` / `getItemMeta` props and re-asserted against `role="treeitem"` (plus 4 new cases for arrow-key emission, arrow `preventDefault`, aria-level / posinset / setsize on couple halves, and tabIndex=-1 when not the focused item); (d) the existing `src/__tests__/genealogyTreeA11y.test.tsx` re-asserted against `role="tree"` and gained one case pinning the roving-tabindex invariant (exactly one treeitem at tabIndex=0, all others at -1). 564 tests pass / 5 skipped (was 535/5; +29); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/utils/treeLayout.ts`, `src/components/TreeNode.tsx`, `src/components/GenealogyTree.tsx`, `src/__tests__/treeNode.test.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `src/__tests__/treeNavigation.test.ts` (new), `src/__tests__/genealogyTreeKeyboard.test.tsx` (new), `TODOS.md`.
