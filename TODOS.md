...
**In priority order (refreshed 2026-05-27):**
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).**~~ Done 2026-05-27 on `claude/keen-newton-4p5sZ`. Upgraded the tree to W3C tree-widget semantics (`role="tree"` on the wrapper `<g>`; `role="treeitem"` on every interactive person `<g>`, replacing `role="button"`) with arrow-key roving tabindex. New pure utility `src/utils/treeNavigation.ts` builds a `NavIndex` from the layout (per-person parent / firstChild / prev / next links plus a DFS-ordered id list). GenealogyTree owns the `focusedPersonId` state, handles `ArrowUp` / `ArrowDown` (prev / next in DFS order), `ArrowLeft` (parent), `ArrowRight` (first child), `Home` / `End` (first / last), and imperatively focuses the resolved treeitem via `data-person-id` lookup. Roving tabindex is preserved across mouse clicks via an `onFocus` delegate on the tree root. See Completed log.
...
- 2026-05-27 — **1.6.a Arrow-key navigation on the D3 tree (W3C tree-widget semantics).** Upgraded `src/components/GenealogyTree.tsx` from `role="group"` to `role="tree"` and every interactive person `<g>` in `src/components/TreeNode.tsx` from `role="button"` to `role="treeitem"` with roving tabindex. New pure utility `src/utils/treeNavigation.ts` exposes `buildNavIndex(layout): { byId: Map<string, NavEntry>, ordered: string[] }` where each `NavEntry` carries `parent` (parent layout's primary id), `firstChild` (first interactive id of the first child layout that has any), and `prev` / `next` linking adjacent ids in DFS order. The synthetic family-root layout is treated as a passthrough so its direct children resolve to `parent: undefined`. Couples contribute two adjacent ids that share the same `parent` and `firstChild` but are linked to each other via `prev` / `next`, so arrow-down naturally steps through the spouse before descending. GenealogyTree owns `focusedPersonId` state (seeded to the first DFS-ordered id, re-seeded via the render-time prev-tracking pattern when the tree changes and the previously focused id is no longer present) and a `shouldFocusRef` flag that guards an effect calling `.focus()` on the new treeitem (so unrelated re-renders, e.g. dims churn from `ResizeObserver`, do not steal focus back). Key handler on the wrapper `<g role="tree">`: `ArrowDown`/`ArrowUp` move along the DFS chain (`entry.next` / `entry.prev`), `ArrowRight` descends to `entry.firstChild`, `ArrowLeft` ascends to `entry.parent`, `Home` / `End` jump to the first / last id in `ordered`. `preventDefault` is only called when the handler actually has a target, so unhandled cases (Up at the root, Left at the root) let the browser keep using the key. `onFocus` delegate on the same wrapper resolves `event.target.closest("[data-person-id]")` and syncs `focusedPersonId` when the user clicks or Tabs onto a non-focused treeitem. TreeNode receives a `focusedPersonId?: string | null` prop (undefined defaults to "tabbable" so isolated unit tests still pass), computes `tabIndex` per half via `shouldBeTabbable`, exposes `data-person-id` on every interactive `<g>` so the parent's querySelector lookup can find it, and emits `aria-expanded="true"` on treeitems whose layout node has children. Per-node `focusedPersonId` derivation in the parent (`isThisFocused ? focusedPersonId : null`) preserves React.memo: only the node losing focus and the node gaining focus re-render across an arrow key press, the other 200+ skip. CSS focus styling (`.tree-node-interactive:focus-visible > rect/circle` stroke change) was already in place from the prior TreeNode keyboard pass and continues to apply since the class did not change. Coverage: 9 new Vitest cases in `src/__tests__/treeNavigation.test.ts` (empty tree, single root, parent / firstChild chain, prev / next across siblings and grandchildren, couple shares parent + firstChild, primary id used as parent reference for descendants, synthetic-root passthrough, walks past intermediate non-interactive layouts, full mixed traversal), 11 new Vitest cases in `src/__tests__/genealogyTreeKeyboard.test.tsx` (initial roving tabindex seed, ArrowDown / ArrowUp DFS step, ArrowRight to first child, ArrowLeft to parent from a couple-half, Home / End boundary jumps, preventDefault per-key with isolated fixtures, no preventDefault when no target, ignored non-nav keys, Enter still activates the focused treeitem, click-then-focus updates the roving tabindex via the onFocus delegate), plus 4 new a11y assertions in `src/__tests__/genealogyTreeA11y.test.tsx` (role=tree on the wrapper with arrow-keys hint in the label, exactly one tabindex=0 treeitem under the tree root, every treeitem carries data-person-id, aria-expanded=true on parents and absent on leaves) and updated role assertions across the existing 9 TreeNode test cases (`button` → `treeitem`). 573 tests pass / 5 skipped (was 560/5; +13); `yarn lint` clean (0/0); `yarn build` green. Files: `src/utils/treeNavigation.ts` (new), `src/__tests__/treeNavigation.test.ts` (new), `src/__tests__/genealogyTreeKeyboard.test.tsx` (new), `src/components/GenealogyTree.tsx`, `src/components/TreeNode.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `src/__tests__/treeNode.test.tsx`, `TODOS.md`. Branch: `claude/keen-newton-4p5sZ`. **Deferred:** `aria-level` / `aria-posinset` / `aria-setsize` on each treeitem would let screen readers announce depth and position. The current layout does not surface depth or sibling index to TreeNode, so threading them through would require either passing depth from `flattenNodes` or another small layout pass. Out of scope here; can ship as a small follow-up once 1.6.d (axe DevTools) flags it.
