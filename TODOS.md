...
### ~~P0-4. Seed route has no authentication~~ Done 2026-05-03
...
**Fix shipped:** Took option (b) per the recommendation. Added `isDevOnlyAllowed()` helper that returns `process.env.NODE_ENV === "development"`, and a `notFoundResponse()` helper that returns `new NextResponse(null, { status: 404 })`. Both `POST` and `DELETE` short-circuit with 404 when the gate fails, before any env-var check or Supabase call. In production, Vercel/Next.js sets `NODE_ENV=production`, so the route is indistinguishable from a non-existent path. Vitest's `NODE_ENV=test` also returns 404 (correct — the route is dev-only). New `src/__tests__/seedRoute.test.ts` covers 9 cases: POST/DELETE 404 in production, POST 404 in test, POST 404 with NODE_ENV undefined, empty 404 body for both methods, POST/DELETE 200 happy paths in development with mocked Supabase REST, and the existing missing-env-var 500 still firing in development. All 339 tests pass; lint clean; `yarn build` green; `/api/seed` still appears as a dynamic route in the build output (no Next.js side effects).
**Follow-ups:** None. P0-5 (convert-image route auth) and P0-6 (PostgREST filter injection) remain as separate tickets in this section.

### P0-5. Convert-image route has no authentication
...
**Status:** Split into three sub-tasks. 1.2.a (reactions) shipped 2026-04-29; 1.2.b (comments) and 1.2.c (email digest) remain unchecked.
...
- [ ] **1.2.b Comments.** New table `memory_comments (id, memoryId, userId, body, createdAt, parentCommentId nullable)`; threaded one level deep; UI + RLS mirror reactions but allow row-owner edit/delete. Estimated 3-4h.
- [ ] **1.2.c Email digest.** Daily/weekly batch via Resend that pings the memory's `createdBy` when reactions or comments come in. Per-user `notification_prefs` jsonb on `app_users` for opt-out. Estimated 2h. Naturally bundles with task 1.3 cron infra.
...
### T-12. GenealogyTree SVG node memoization
...
- 2026-05-03 — **P0-4 Seed route dev-only gate.** Added `isDevOnlyAllowed()` (`process.env.NODE_ENV === "development"`) + `notFoundResponse()` helpers in `src/app/api/seed/route.ts`; both `POST` and `DELETE` now return an empty 404 before any env-var check or Supabase call when not running under `next dev`. Closes the P0-4 audit finding (an unauthenticated caller could previously seed or wipe data via the service role). New `src/__tests__/seedRoute.test.ts`: 9 cases — POST 404 in production / test / undefined NODE_ENV, DELETE 404 in production, 404 body is empty for both methods, POST/DELETE 200 in development with mocked Supabase REST, and the pre-existing 500 path for missing `SUPABASE_SERVICE_ROLE_KEY` still firing in development. 339 tests pass / 5 skipped; `yarn lint` clean; `yarn build` green; `/api/seed` still listed as a dynamic route in the build output. Files: `src/app/api/seed/route.ts`, `src/__tests__/seedRoute.test.ts` (new), `TODOS.md`.
