...
- ~~Structured oral-history prompts — shipped 2026-05-14 (1.4)~~
...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ — Done 2026-05-14

See Completed log. **Open follow-ups:**

- **1.4.a Surface the prompt on the memory tile.** Memories created from a prompt store `promptId`, but the memory tiles on `/memories`, `/profile/[id]`, and `/timeline` don't show "answered: {question}" yet. +1h.
- **1.4.b AI-generated follow-up prompts.** Optional Claude API hop after the user answers ("Tell me more about the friend you mentioned"). +4h. Punted out of the base scope.
- **1.4.c Admin curation UI.** Today, adding prompts requires a new migration (writes are service-role-only). A `/admin/prompts` page that lets admins add/retire prompts via the service-role endpoint is a deferred curation tool. +2h.
- **1.4.d Standalone `/prompts` browsing page.** A page that lists all prompts by category and lets the user pick any prompt to answer (not just today's) — useful for a more "intentional" session. +2h.
- **1.4.e Track which prompts a user has answered.** Today the daily prompt is deterministic by `(userId, ymd)` and can repeat across weeks. A "don't re-show prompts I've already answered" filter would require joining against `memories.promptId` server-side. +2h.
...
- 2026-05-14 — **1.4 Guided story prompts ("Ask Grandma" mode).** Migration `20260514_story_prompts.sql` (+ rollback) creates `public.story_prompts (id, category, question, createdAt)` with a `check (category in ('childhood','career','love','faith','travel','holidays','pets'))`, a `check (length(btrim(question)) between 5 and 500)`, and a `unique (category, question)` constraint so the seed block (`on conflict (category, question) do nothing`) is idempotent across re-runs. The same migration seeds 70 prompts (10 per category, family-friendly), adds nullable `memories.promptId uuid references public.story_prompts(id) on delete set null` (with a partial index on `where "promptId" is not null` for back-link lookups), and gates RLS allowlist-only: SELECT for any approved user, writes service-role-only because the catalogue is curated, not user-generated. New `StoryPrompt` model + `STORY_PROMPT_CATEGORIES` enum. New `listStoryPrompts()` helper in `src/lib/db.ts` (ordered by category). New pure util `src/utils/dailyPrompt.ts` with `pickDailyPrompt(prompts, userId, date)` and `toYmd(date)` — FNV-1a 32-bit hash over `${userId}|${YYYY-MM-DD}` modulo prompt count, so the same user sees a stable prompt all day and a different one tomorrow, while two users typically see different prompts on the same day. `toYmd` uses local-time `getFullYear/getMonth/getDate` rather than `toISOString` to avoid UTC drift shifting the day for users east/west of GMT around midnight. New `StoryPromptCard` (`"use client"`) home widget reads the catalogue, picks today's prompt, renders "A question for you today" + the question + category, and exposes "Answer with a memory" / "Skip for today" buttons. The skip marker is `family_legacy_prompt_skip_<userId> = YYYY-MM-DD` in localStorage (`useSyncExternalStore` with a module-level listener `Set` for the React 19 anti-`set-state-in-effect` pattern); a stale marker from yesterday is ignored. Skip is per-user, so two users sharing a browser do not cross-contaminate. `AddMemoryModal` gains optional `promptId?` + `promptQuestion?` props; when set, it renders a "Answering today's question" banner above the form and writes `promptId` into the new memory row. Wired into `src/app/page.tsx` between the greeting and the hero spread. Coverage: 14 migration regex assertions in `storyPromptsMigration.test.ts` (table shape, category check, length check, unique constraint + on-conflict idempotency, RLS-gated select + service-role-only writes, `promptId on delete set null`, partial index, no-destructive guarantee, ≥50 seeded prompts across all 7 categories, `pgcrypto` extension, rollback drops everything but does not drop `memories`); 7 pure cases in `dailyPrompt.test.ts` (`toYmd` formatting + zero-pad, empty-list → null, deterministic per `(user, day)`, rotates across days, varies across users, always-in-bounds for 60 days × 7-prompt list, single-prompt list, stability across reruns); 3 db-helper cases in `storyPromptsDb.test.ts` (`/rest/v1/story_prompts` GET + `order=category.asc`, empty-rows passthrough, error propagation); 11 widget cases in `storyPromptCard.test.tsx` (renders question + category after fetch, empty container while loading, empty container with no prompts, empty container on fetch error with `console.error` swallowed, opens stubbed `AddMemoryModal` with `promptId` + `promptQuestion`, Skip hides the card and writes today's marker, persisted marker hides the card on remount, stale marker from yesterday is ignored, per-user marker isolation, same prompt across two mounts on the same day, `onAnswered` fires + modal unmounts after save). 565 tests pass / 5 skipped (was 522/5; +43); `yarn lint` clean (0/0); `yarn build` green. Files: `supabase/migrations/20260514_story_prompts.sql` (new), `supabase/migrations/20260514_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/models/Memory.ts`, `src/lib/db.ts`, `src/utils/dailyPrompt.ts` (new), `src/components/StoryPromptCard.tsx` (new), `src/components/AddMemoryModal.tsx`, `src/app/page.tsx`, `src/__tests__/storyPromptsMigration.test.ts` (new), `src/__tests__/dailyPrompt.test.ts` (new), `src/__tests__/storyPromptsDb.test.ts` (new), `src/__tests__/storyPromptCard.test.tsx` (new), `SUPABASE_SETUP.md`, `TODOS.md`. Deferred (split into sub-items 1.4.a-e under the original task): surface the prompt on memory tiles (+1h), AI-generated follow-ups (+4h), admin curation UI (+2h), standalone `/prompts` browsing page (+2h), filter prompts the user has already answered (+2h).
