...
- ~~**1.5.b Half-sibling labeling**~~ Done 2026-05-30. The sibling branch in `bloodLabel` now consults a `sharedParentCount(aId, bId, peopleById)` helper that counts ids present in both `Person.parentIds` arrays. With ≥2 shared the label stays "Sibling"; with ≤1 it becomes "Half-sibling". Missing parents on either side count as non-shared, so a child with one recorded parent who shares that parent with another child is correctly labeled "Half-sibling" rather than over-claiming a full-sibling relationship. The previously-pinned `relationship.test.ts` case ("documented limitation") was flipped to expect "Half-sibling", plus three new cases cover (a) the partial-parentage edge, (b) both sides recording only the shared parent, and (c) `commonAncestorId` still pointing at the shared parent. Profile-page chip ("Your Sibling" → "Your Half-sibling") inherits the change automatically. See Completed log.
...
- 2026-05-30 — **1.5.b Half-sibling labeling in the relationship calculator.** The profile-page "Your {label}" chip was rendering "Your Sibling" for half-siblings — incorrect on every profile page where the viewing user only shares one parent with the displayed person. Fix in `src/utils/relationship.ts`: new pure `sharedParentCount(aId, bId, peopleById)` helper counts ids present in both `Person.parentIds` arrays (ids alone — parents that are missing from the lookup map still count). `bloodLabel` now takes a third `sharedParents` argument, used only in the `stepsA === 1 && stepsB === 1` branch: ≥2 stays "Sibling", ≤1 becomes "Half-sibling". `findRelationship` only computes the count when both BFS legs land at depth 1 (every other branch passes 0, since the half-/full- distinction is meaningless for parent/child, aunt/uncle, niece/nephew, cousins, etc.). Conservative on missing data: a child with one recorded parent shared with another child whose second parent is unknown is labeled "Half-sibling" rather than over-claiming a full-sibling relationship that the data doesn't support. The previously-pinned `relationship.test.ts` case ("documented limitation — labeled as Sibling") was flipped to expect "Half-sibling", plus three new cases cover (a) the partial-parentage edge where one side has only one recorded parent, (b) both sides recording only the shared parent, and (c) `commonAncestorId` still pointing at the shared parent for half-siblings. Top-of-file limitations comment in `relationship.ts` updated to document the new behavior and the missing-parent treatment. Profile-page chip inherits the change automatically — no `profile/[id]/page.tsx` change needed. 553 tests pass / 5 skipped (was 550 / 5; +3); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/utils/relationship.ts`, `src/__tests__/relationship.test.ts`, `TODOS.md`. Branch: `claude/vigilant-cannon-E0yIv`. **Deferred:** in-law relationships (1.5.a — chain a spouse hop on either side), step / adoptive / foster distinctions (1.5.c — needs `relationships.subtype` joined back in rather than the denormalized `parentIds`), standalone `/relationships` page with a two-person picker (1.5.d). The "Half-{cousin / aunt / uncle / etc.}" generalization for collateral relatives further down the tree was *not* attempted; the TODO scope is specifically siblings.
