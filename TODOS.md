...
### ~~T-14. Webhook new-user route: duplicate PostgREST filter param~~ ✅ Done 2026-05-17
...
- 2026-05-17 — **T-14 Webhook new-user route: collapse duplicate PostgREST `email` filter into a single `and()` expression.** `src/app/api/webhooks/new-user/route.ts` previously appended the `email` query param twice (`email=not.is.null` + `email=neq.${newUserEmail}`) and relied on PostgREST AND-ing duplicate keys — defined behavior today, but fragile and non-obvious. Replaced with a single `and=(email.not.is.null,email.neq."{escaped}")` parameter; the new user's email is now wrapped in double quotes + run through the existing `escapePgrstString` helper from `src/utils/pgrstEscape.ts`, so a payload value containing `"`, `\`, `,`, or `)` can no longer break out of the filter token (the route can't assume an upstream Supabase Auth webhook payload is well-formed). No behavior change at the SQL level — the recipient query still returns "people with a non-null email, excluding the new user's email". 2 new Vitest cases in `src/__tests__/webhookNewUser.test.ts` pin the URL shape: (a) `searchParams.getAll("email")` is empty (no duplicate key regression), `select=email`, and `and=(email.not.is.null,email.neq."new@example.com")`; (b) a payload with `"` + `\` in the email survives the round-trip through the `and(...)` expression escaped (`email.neq."weird\"\\@example.com"`). 529 tests pass / 5 skipped (was 527/5; +2); `yarn lint` clean (0/0); `yarn build` green. Files: `src/app/api/webhooks/new-user/route.ts`, `src/__tests__/webhookNewUser.test.ts`, `TODOS.md`. Branch: `claude/vigilant-cannon-FWjCe`.
