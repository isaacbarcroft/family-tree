...
3. **1.6.d Run [axe DevTools](https://www.deque.com/axe/devtools/)** and fix all critical / serious issues. Best done after 1.6.a / 1.6.b so the tree and landmark changes are reflected in the report.
...
- ~~**1.6.c Focus management after tree node click.**~~ Done 2026-05-22 on `claude/vigilant-cannon-GZHQi`. See Completed log.
...
- 2026-05-22 — **1.6.c Focus management after profile-page navigation.** Keyboard / screen-reader users who activate a person from the D3 tree (or any other `router.push("/profile/[id]")` call site — `AddMemberModal` after create, family-constellation cards, NavBar search results) used to land back at `<body>` once the destination route mounted, forcing a full re-tab through the NavBar before reaching the new page's content. `src/app/profile/[id]/page.tsx` now owns a `headingRef: useRef<HTMLHeadingElement>(null)` that the `Hero` subcomponent attaches to the first `<h1>` (the one rendering `person.firstName` / `middleName`), with `tabIndex={-1}` so the heading is programmatically focusable without entering the tab order. A `useEffect` deps-on `[loading, editing, person]` calls `headingRef.current?.focus({ preventScroll: true })` after the person record finishes loading; `preventScroll: true` lets Next.js' built-in route-change scroll-to-top behavior run uncontested. Three early-returns gate the call: still loading, in edit mode (Hero is unmounted so the ref is null anyway), or no person record. Focus revisits on `person.id` change too, so clicking a family-constellation card on one profile and landing on another refocuses the new name. The `*:focus-visible` outline already declared in `globals.css:135` (2px `var(--sage-deep)`) handles the visual indicator; the browser's `:focus-visible` heuristic suppresses it for mouse-driven nav, so sighted mouse users do not see a stray outline. Coverage: 4 new Vitest cases in `src/__tests__/profilePageFocus.test.tsx` — (1) the rendered h1 carries `tabindex="-1"`, (2) `document.activeElement` becomes the h1 once `getPersonById` resolves, (3) focus stays on `<body>` while the person record is still loading and only moves after the promise resolves (regression pin for the loading-gate early-return), (4) mounting the page directly into edit mode (`?edit=true`) does not move focus and the Hero h1 is not rendered (regression pin for the editing-gate early-return). 544 tests pass / 5 skipped (was 540/5; +4); `yarn lint` clean (0/0); `yarn build` green. Files: `src/app/profile/[id]/page.tsx`, `src/__tests__/profilePageFocus.test.tsx` (new), `TODOS.md`. Branch: `claude/vigilant-cannon-GZHQi`. Deferred: applying the same heading-focus pattern to `/family-tree`, `/families/[id]`, `/memories`, `/events`, `/places`, `/timeline` — same accessibility win on those routes, but out of scope for this PR; tracked implicitly under the open 1.6.d axe pass.
