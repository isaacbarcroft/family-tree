...
`src/components/ProfileAvatar.tsx` already uses `next/image`. Memory thumbnails on `/memories` and `/profile/[id]` (plus the home page featured-memory hero, the timeline, and the family-tree page) all render through `src/components/ui/PhotoFrame.tsx`. **2026-05-12 update:** `PhotoFrame` now lazy-loads (`loading="lazy"`), sets `referrerPolicy="no-referrer"`, and routes Supabase HEIC URLs through `toDisplayImageUrl` internally — restoring the defaults the prior `MemoryImage` component had before the UI refresh, and removing the per-caller foot-gun where forgetting to wrap a HEIC URL meant Safari-only photos failed to render in other browsers. **Still open:** swap `<img>` for `next/image` (with explicit `sizes`) inside `PhotoFrame` so the browser gets format-negotiated AVIF/WebP and intrinsic dimensions; generate thumbnail variants on upload (sharp in an edge function or server action).
**Effort:** 2h remaining
...
- 2026-05-12 — **T-3 (partial) — restore PhotoFrame lazy-load/no-referrer/HEIC defaults; delete dead `MemoryImage`.** Discovery: `src/components/MemoryImage.tsx` (and its `src/__tests__/memoryImage.test.tsx`) was only imported by its own test — the UI refresh (PR #28) replaced it with `PhotoFrame` (`src/components/ui/PhotoFrame.tsx`) across every memory thumbnail call site, but three defaults the prior component carried did not move over: (1) `loading="lazy"` on the `<img>`, so every PhotoFrame instance was eager-loaded — measurable cost on `/memories` and `/profile/[id]` where pages render N tiles in a grid; (2) `referrerPolicy="no-referrer"`, so cross-origin Supabase Storage URLs leaked the full media path via the `Referer` header on every image request; (3) routing through `toDisplayImageUrl` for HEIC handling, which meant callers had to remember to wrap any user-uploaded photo URL or Safari-only HEIC photos failed to render in Chrome/Firefox/Edge. Fix: added all three defaults inside `PhotoFrame` so they apply automatically; `toDisplayImageUrl` is idempotent on already-transformed URLs and pass-through on `blob:` previews, so existing `toDisplayImageUrl(...)` wraps at call sites stay safe and don't need to be removed in this PR. Deleted `MemoryImage.tsx` + its test (45 + 60 lines of dead code) and updated five stale "matching MemoryImage / ProfileAvatar" comment references in `ProfileAvatar.tsx`, `ResidencesEditor.tsx`, `AddMemberModal.tsx`, `NavBar.tsx`, `ui/Avatar.tsx` to reference `ProfileAvatar` only. Coverage: added 4 new Vitest cases to `src/__tests__/ui/PhotoFrame.test.tsx` (lazy-load attribute, `referrerpolicy` attribute, HEIC URL routed to the render-image endpoint with `?format=jpeg&quality=85`, non-HEIC URL passes through unchanged, `blob:` URL passes through unchanged). 527 tests pass / 5 skipped (was 524/5; +3 net after deleting `memoryImage.test.tsx`); `yarn lint` clean (0/0); `yarn build` green. Files: `src/components/ui/PhotoFrame.tsx`, `src/__tests__/ui/PhotoFrame.test.tsx`, `src/components/MemoryImage.tsx` (deleted), `src/__tests__/memoryImage.test.tsx` (deleted), `src/components/ProfileAvatar.tsx`, `src/components/ResidencesEditor.tsx`, `src/components/AddMemberModal.tsx`, `src/components/NavBar.tsx`, `src/components/ui/Avatar.tsx`, `TODOS.md`. Deferred (T-3 remainder): swap `<img>` for `next/image` inside `PhotoFrame` for format negotiation + intrinsic dimensions; generate thumbnail variants on upload.
