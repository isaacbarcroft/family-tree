...
- ~~`alt` on every `<img>` (especially `ProfileAvatar`)~~ — verified done 2026-05-12. `MemoryImage`, `ProfileAvatar`, `ui/Avatar`, and `ui/PhotoFrame` each render their `<img>` with an `alt` prop sourced from the caller.
- ~~Focus trap in every modal (`AddMemoryModal`, `AddEventModal`, `AddFamilyModal`, `AddMemberModal`, `ImportGedcomModal`, `ConfirmDialog`)~~ — verified done 2026-05-12. All true modals (`Add*Modal`, `ImportGedcomModal`, `WelcomeModal`) render through the shared `src/components/Modal.tsx`, which already implements tab trap, escape-to-close, initial-focus, and focus restoration. `ConfirmDialog` is an inline yes/no control rather than a real modal (no backdrop, no `role="dialog"`).
- ~~Semantic HTML: `<main>`, `<nav>`, `<section>`, `<article>` on page shells~~ — verified done 2026-05-12. `src/app/layout.tsx` wraps children in `<main>`, `NavBar` renders inside a `<nav>`, and the home page already uses `<section>` / `<header>` for its content shells.
- ~~Visible `:focus-visible` outline in Tailwind theme~~ — verified done 2026-05-12. `src/app/globals.css` line 135 sets `*:focus-visible { outline: 2px solid var(--sage-deep); outline-offset: 2px; }` globally, with the SVG-specific override at line 225 for `.tree-node-interactive`.
- `aria-label` on icon-only buttons — NavBar's theme toggle, account menu, and mobile menu trigger already carry `aria-label`. Sweep the rest (`UngeocodedSidebar` retry button, any other `<button>` whose only child is an `<Icon>` / SVG) as a follow-up.
- Keyboard navigation on the D3 tree (arrow keys to move between nodes — beyond Enter / Space). Tab order already walks the focusable groups in DOM order; arrow-key panning between siblings / parents / children is the next escalation.
- ~~SVG edges are invisible to screen readers~~ — done 2026-05-12. Edge `<path>` layer in `GenealogyTree.tsx` now lives inside a `<g aria-hidden="true">` wrapper so assistive tech walks straight from one person-button to the next without hearing the relationship lines as unlabeled graphics. The `<svg>` itself carries an `aria-label` of the form "Family tree, N people. Use Tab to move between people, then press Enter or Space to open their page." with N derived from the rendered nodes (couple = 2, single = 1, synthetic family-root label = 0).
- No focus management after tree node click — the user lands on the profile page with no focus indicator. Still outstanding.
...
- 2026-05-12 — **1.6 (next gap) — GenealogyTree SVG screen-reader accessibility.** Three follow-ups under 1.6: (1) the root `<svg>` in `src/components/GenealogyTree.tsx` was unlabeled, so assistive tech announced an unnamed graphic; (2) the relationship-line `<path>` elements were direct children of the same `<g>` as the interactive person buttons, so a screen reader walking the tree would read each as an unlabeled graphic between people; (3) the four other 1.6 follow-ups (alt text, modal focus trap, semantic HTML on page shells, `:focus-visible` outline) all reviewed and confirmed already in place. Changes in this session: derived a `personCount` memo over the laid-out nodes (couple = 2, single = 1, synthetic family-root label = 0), composed a `treeAriaLabel` of the form "Family tree, N people. Use Tab to move between people, then press Enter or Space to open their page." (singular "1 person" when N is 1), and set it as the SVG's `aria-label`. Wrapped the edge `<path>` map in a `<g aria-hidden="true">` so the entire relationship-line layer is hidden from assistive tech without affecting visual rendering; the TreeNode person-buttons stay direct children of the parent `<g ref={gRef}>` so `aria-hidden` does not propagate into any focusable element. Coverage: 7 new Vitest cases in `src/__tests__/genealogyTreeA11y.test.tsx` (singular "1 person" label for a one-person tree, plural "N people" for a multi-person tree, couple node counted as 2, synthetic-root tree reports "0 people", aria-label includes "Tab" + "Enter or Space" interaction hint, decorative edges live inside an `aria-hidden="true"` group and no `<path>` sits at the SVG's top level, the aria-hidden wrapper does not contain any `[role='button']` so every TreeNode button stays reachable). 534 tests pass / 5 skipped (was 527/5; +7); `yarn lint` clean (0 errors / 0 warnings); `yarn build` green. Files: `src/components/GenealogyTree.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx` (new), `TODOS.md`. Branch: `claude/keen-newton-SYAWA`. Deferred under 1.6: arrow-key navigation between nodes (Tab order already works in DOM order), focus management after tree-node click (lands on `/profile/[id]` with no focus indicator), `aria-label` sweep of remaining icon-only buttons outside NavBar, axe DevTools pass.
