...
2. **1.6 Accessibility** — `TreeNode.tsx` keyboard support shipped 2026-05-10; remaining scope (modal focus traps, alt text, semantic landmarks, axe pass) is the highest-impact gap left in Phase 1.
...
- ~~Structured oral-history prompts — shipped 2026-05-15~~
...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ — Done 2026-05-15

See Completed log. Base scope shipped (story_prompts table + 60 seeded prompts + home widget + memories.promptId link). **Open follow-ups:**

- **1.4.a AI-generated follow-up prompts** based on a relative's answer (Claude API call). Deferred from the original scope to keep the first ship reviewable. +4h.
- **1.4.b Per-user prompt rotation.** Today the daily prompt is deterministic per calendar day and shared across the family — same question for everyone. A per-user rotation that skips prompts a relative has already answered (`memories.promptId is not null` per user) is the natural next step. +2h.
- **1.4.c Single CTA on the home widget.** The original spec called for "Answer with text / Answer with voice" as two CTAs; landed as one "Answer this" button because browser policy blocks `getUserMedia` from auto-firing without an in-modal user gesture, so a "voice" CTA can't actually start the recorder on its own. The modal still surfaces both flows. Worth revisiting if we add a "scroll to recorder" affordance. +1h.
- **1.4.d Browse-by-category prompt library.** A `/prompts` page that lists every prompt grouped by category, with an "Answer this" CTA per row, would let users dive into specific themes (e.g., "show me all the food prompts"). +2h.
- **1.4.e Surface "answered prompts" on profile pages.** With `memories.promptId` now wired, a person's profile could show "Questions {firstName} has answered" as a sidebar list. +2h.
...
- 2026-05-15 — **1.4 Guided story prompts ("Ask Grandma" mode) — base scope.** Migration `20260515_story_prompts.sql` (+ rollback) adds `public.story_prompts (id, text, category, createdAt, deletedAt)` with `text` unique (so the seed is idempotent), a category check constraint pinned to the curated set (childhood, family, love, faith, career, holidays, travel, food, pets, milestones), a partial `(category) where deletedAt is null` index, and a 60-row `on conflict (text) do nothing` seed of starter prompts spread across all ten categories. RLS uses the `app_users` allowlist: SELECT for any approved user; INSERT / UPDATE / DELETE are admin-only (service role bypasses for the seed and any future curator tooling). The same migration adds nullable `memories.promptId uuid references story_prompts(id) on delete set null` plus a partial `(promptId) where promptId is not null and deletedAt is null` index, so retiring a prompt later doesn't orphan the memories that answered it. New `StoryPrompt` model + `STORY_PROMPT_CATEGORIES` const tuple. New pure `src/utils/dailyPrompt.ts` exports `dateKeyFor(date)` (local YYYY-MM-DD, zero-padded) and `selectDailyPrompt(prompts, dateKey)`: filters out soft-deleted rows, sorts by id for stability across re-renders, and picks an index via a tiny djb2-style string hash of the date key. The selector is deterministic per calendar day and shared across the family — same question for everyone — which keeps the first ship simple. Per-user rotation that excludes already-answered prompts is tracked as 1.4.b. New `src/lib/db.ts` helpers `listStoryPrompts()` (filters `deletedAt is null`) and `getStoryPromptById(id)` (uses `.maybeSingle()` so missing → null instead of throw). New `DailyPromptCard.tsx` renders inside the home page's existing editorial layout between the hero spread and the lower grid; surfaces today's prompt with one "Answer this" CTA that deep-links to `/memories?prompt=<id>`. The card uses `aria-labelledby` pointing at the heading so screen readers land on it as a labeled landmark. (One CTA instead of "Answer with text / Answer with voice" — see 1.4.c — because browser policy blocks `getUserMedia` from auto-firing without an in-modal user gesture, so a "voice" route can't actually start the recorder on its own.) `AddMemoryModal.tsx` accepts `prompt?: StoryPrompt | null`, renders a labeled banner above the form when set ("Answer a prompt" heading + the question text), pre-fills the title input with the prompt text so the user starts mid-answer instead of staring at a blank field, and stores `promptId` on submit. `/memories/page.tsx` reads `?prompt=<id>` from `useSearchParams`, fetches the prompt with `getStoryPromptById`, opens the modal automatically, and clears the query param via `router.replace("/memories")` when the modal closes — so a refresh / back-navigation doesn't keep re-opening the modal. The page is wrapped in a `Suspense` boundary because `useSearchParams` requires it under Next.js 16. Coverage: 13 migration regex assertions (table shape, category check + all 10 values, unique-text constraint, on-conflict seed, deletedAt column, RLS gates including the no-authenticated-write pin, all four policy bodies, memories.promptId column with set-null on delete, partial index, ≥50 seed-row count, no-destructive); 2 rollback assertions (drops every policy + table + the memories.promptId column + its index); 7 dailyPrompt cases (date key formatting + zero-pad, empty-list null, all-soft-deleted null, soft-delete filtering, determinism across calls, order-independence via internal sort, adjacent-day spread, never-out-of-list); 5 db cases (`listStoryPrompts` deletedAt filter + return shape, `getStoryPromptById` missing-row null + present-row return + 500 propagation); 4 DailyPromptCard cases (null-prompt renders nothing, text + category visible, CTA href shape, aria region label); 4 AddMemoryModal-prompt cases (banner + title pre-fill, default heading without prompt, promptId in submit payload, promptId omitted without prompt). 562 tests pass / 5 skipped (+35 vs the pre-session baseline of 527/5); `yarn lint` clean (0/0); `yarn build` green. `SUPABASE_SETUP.md` lists the new migration and documents the soft-delete-to-retire-a-prompt path. Files: `supabase/migrations/20260515_story_prompts.sql` (new), `supabase/migrations/20260515_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/models/Memory.ts`, `src/utils/dailyPrompt.ts` (new), `src/components/DailyPromptCard.tsx` (new), `src/components/AddMemoryModal.tsx`, `src/app/page.tsx`, `src/app/memories/page.tsx`, `src/lib/db.ts`, `src/__tests__/storyPromptsMigration.test.ts` (new), `src/__tests__/dailyPrompt.test.ts` (new), `src/__tests__/storyPromptsDb.test.ts` (new), `src/__tests__/dailyPromptCard.test.tsx` (new), `src/__tests__/addMemoryModalPrompt.test.tsx` (new), `SUPABASE_SETUP.md`, `TODOS.md`. Deferred: 1.4.a AI follow-ups (+4h), 1.4.b per-user rotation that excludes already-answered prompts (+2h), 1.4.c split-CTA on the home widget (+1h), 1.4.d browse-by-category prompt library page (+2h), 1.4.e "Questions {firstName} has answered" sidebar on profile pages (+2h).
