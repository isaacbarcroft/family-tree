...
1. **1.6 Accessibility** — `TreeNode.tsx` keyboard support is done; remaining: modal focus traps, semantic HTML, `aria-label` on icon-only buttons, D3 arrow-key navigation, axe pass.
...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ — Done 2026-05-16

See Completed log. **Open follow-ups:**

- **1.4.a AI-generated follow-up prompts.** After the user answers a prompt, call Claude API to suggest a related follow-up question (e.g. they answered "first car" → "What was the longest road trip you took in that car?"). +4h. Reference: Claude API.
- **1.4.b Skip / refresh affordance on the home widget.** Right now the same prompt sticks all day. A "Show me another" button (with a server-stored "recently shown" list to avoid repeats within a window) would help relatives who freeze on the first prompt they see. +1h.
- **1.4.c Category preferences.** A `/settings` switch that lets a user opt out of a category (e.g. someone uncomfortable with faith). Today `pickDailyPrompt` filters by nothing, so this is a one-line extension on the read side once a `categoryPrefs` column exists. +2h.
- **1.4.d "How would you answer this?" digest view.** Surface all prompts that have been answered (or none) so relatives can see what's been recorded and what hasn't. +3h.
...
- 2026-05-16 — **1.4 Guided story prompts ("Ask Grandma" mode).** New migration `20260516_story_prompts.sql` (+ rollback) creates `public.story_prompts (id uuid pk, category text check (childhood / career / love / faith / travel / holidays / pets), text text 1-500 chars after btrim, createdAt timestamptz)` with `unique (category, text)` so the seed `insert ... on conflict do nothing` is idempotent. Indexes on `category` for filtered reads. Same migration adds a nullable `promptId uuid` column to `public.memories` with `references public.story_prompts(id) on delete set null` plus a partial index `where "promptId" is not null` so historic memories keep their content if a prompt is removed from the catalog. RLS uses the existing `is_approved_user()` / `is_admin_user()` helpers: SELECT for any approved user, INSERT / UPDATE / DELETE for admins only (the catalog is curated, not user-generated). Seeded 57 prompts across the seven categories. New `StoryPrompt` model + `listStoryPrompts()` db helper in `src/lib/db.ts`. New pure `src/utils/storyPrompt.ts` exports `dateKey(date)` (local-time `YYYY-MM-DD`) and `pickDailyPrompt(prompts, key, userId?)` — deterministic FNV-1a hash modulo a sorted-by-id catalog so (a) the prompt is stable for the whole day, (b) the prompt is stable across re-renders, (c) two relatives looking at the home page the same day see different prompts when the optional `userId` salt is provided. New `StoryPromptCard.tsx` component renders on the authenticated home page (between the hero spread and the lower grid) with the daily question and two CTAs — "Answer with text" opens `AddMemoryModal` with the prompt prefilled, "Answer with voice" opens the modal with a new `autoStartRecording={true}` prop that calls `startRecording()` once on mount (gated by a `useRef` so prop changes do not retrigger). `AddMemoryModal` accepts a new optional `prompt: { id; text }` prop that pre-fills the title input, swaps the heading to "Answer the question", renders a sage-tinted question banner, and forwards `promptId` on the memory payload. Coverage: 16 migration cases (table shape, category check, length check, unique key, indexes, RLS gates, admin-only writes, on-conflict idempotence, all 7 seed categories present, ≥50 prompts seeded, no destructive ops, + 3 rollback cases), 9 utility cases (dateKey local-time padding, pickDailyPrompt empty / determinism / cross-date variation / user salt / array-order stability / single-prompt fallback / immutability / referential identity), 4 db cases (URL contract, payload pass-through, null-row guard, error propagation), 6 component cases (renders prompt + CTAs, modal hidden until click, text CTA passes `autoStartRecording=false`, voice CTA passes `autoStartRecording=true`, onClose hides modal, onCreated hides modal + fires onAnswered), 5 modal cases (title prefilled from prompt, banner + heading render, submit forwards `promptId`, no `promptId` without prompt prop, default heading without prompt). 572 tests pass / 5 skipped (was 527/5; +45); `yarn lint` clean (0/0); `npx tsc --noEmit` clean; `yarn build` green. Files: `supabase/migrations/20260516_story_prompts.sql` (new), `supabase/migrations/20260516_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/models/Memory.ts`, `src/utils/storyPrompt.ts` (new), `src/lib/db.ts`, `src/components/StoryPromptCard.tsx` (new), `src/components/AddMemoryModal.tsx`, `src/app/page.tsx`, `SUPABASE_SETUP.md`, `TODOS.md`, plus 5 new test files. Deferred (1.4.a AI follow-up prompts via Claude API, 1.4.b skip/refresh affordance, 1.4.c category opt-out preferences, 1.4.d "answered prompts" digest view) tracked under 1.4.
