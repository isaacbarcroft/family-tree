...
### P0-4. Seed route has no authentication
...
**Fix:** Either (a) add a `verifyUser` + admin check matching the geocode route pattern, or (b) gate the entire route behind `process.env.NODE_ENV === "development"` so it 404s in production. Option (b) is simpler and sufficient since seeding is a dev-only operation.
**Effort:** 15 min
...
- [ ] **1.2.d Bulk reaction fetch on list pages.** `MemoryReactions` currently fetches per-memory on the profile page (each tile mounts its own component). For large memory lists this is N+1; switch to a single `listReactionsForMemories(ids)` call at the page level and pass `initialReactions` down. Estimated 1h.

**Effort (remaining):** 5-7h.
