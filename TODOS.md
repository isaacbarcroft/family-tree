...
- ~~**1.5.b Half-sibling labeling**~~ Done 2026-05-26 — see Completed log. "Half-sibling" is now emitted by `bloodLabel` when both people list ≥ 2 parents and the overlap is < 2; ambiguous cases (one or both have < 2 parents recorded) keep the existing "Sibling" label.
...
- 2026-05-26 — **1.5.b Half-sibling labeling in the relationship calculator.** `bloodLabel` now distinguishes full siblings from half-siblings instead of collapsing both to "Sibling". `findRelationship` precomputes (a) the count of shared parent ids and (b) each person's known-parent count and passes all three down to `bloodLabel`; the label switches to "Half-sibling" only in the direct-sibling case (stepsA === stepsB === 1) when both people list ≥ 2 parents and the overlap is < 2. Ambiguous data (one or both people have < 2 parents recorded) keeps the existing "Sibling" label, because the missing entry could equally hide a full-sibling link and mislabeling missing data as half-sibling would be worse than under-specific. The result's `kind` ("blood"), `stepsA`, `stepsB`, and `commonAncestorId` are unchanged, so the single caller (`/profile/[id]` chip) and any other future consumer keep their existing data shape — only the rendered label flips for the half-sibling case ("Your Half-sibling" instead of "Your Sibling"). The matching "Known limitations" comment block at the top of `findRelationship` was updated to drop the half-sibling bullet (in-law and step/adoptive bullets remain). Coverage: 6 new Vitest cases in `src/__tests__/relationship.test.ts` — shared-mother half-sibling, shared-father half-sibling, incomplete-data "don't falsely label" pin, both-single-parent ambiguity pin, full-sibling regression pin, and a result-shape pin that asserts `kind` / `stepsA` / `stepsB` / `commonAncestorId` are still consistent with the full-sibling branch. The existing "labels half-siblings as Sibling (documented limitation)" test was rewritten to assert the new label. 545 pass / 5 skip (was 535 / 5; +10); `yarn lint` clean (0/0); `yarn build` green. Files: `src/utils/relationship.ts`, `src/__tests__/relationship.test.ts`, `TODOS.md`. Deferred follow-ups (still tracked under 1.5): 1.5.a in-law relationships, 1.5.c step/adoptive distinction (needs the `relationships.subtype` join), 1.5.d standalone `/relationships` page.
