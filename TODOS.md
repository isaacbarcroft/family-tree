...
1. **T-16** — PhotoFrame lint regression, ~15 min. Restores the 0 errors / 0 warnings baseline.
2. **T-17** — `/api/webhooks/new-user` auth spot check, ~10 min.
3. **T-13** — wrap `GenealogyTree` in `next/dynamic`, ~30 min. Leaflet is already done in `places/page.tsx`.

**Then in priority order:**

4. **1.6 Accessibility** — start with `TreeNode.tsx` keyboard support. Highest-impact remaining gap; older relatives are the target audience.
...
- **Lint regression.** 5 `react-hooks/set-state-in-effect` warnings in `src/components/ui/PhotoFrame.tsx:30` introduced by the UI refresh. Tracked as T-16.
...
- ~~Structured oral-history prompts (1.4) — shipped 2026-05-09~~
...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ — Done 2026-05-09 (PR #50)
...
- Added `story_prompts` plus a nullable `memories.storyPromptId` back-pointer, seeded 56 prompts across childhood, career, love, faith, travel, holidays, and pets.
- Home page now shows a deterministic daily prompt widget with both "Answer with text" and "Answer with voice" flows.
- Prompt answers open `AddMemoryModal` with the prompt context attached, and saved memories persist the linked `storyPromptId`.

**Open follow-ups:**

- **1.4.a AI-generated follow-up prompts.** Use the saved answer as context for a second question after submit. This is explicitly deferred from the base prompt catalog shipping here. +4h.
...
### T-13. Dynamic imports for `GenealogyTree` (Leaflet already done)

**Status:** `src/app/places/page.tsx` already wraps `PlacesMap` in `next/dynamic` with `ssr: false`. The remaining work is doing the same for `GenealogyTree`.
**Files:** Components importing D3 (`d3-selection` + `d3-zoom` ~40KB)
**Problem:** D3 is statically imported, adding ~40KB to the initial bundle on pages that don't render the tree. Doesn't work server-side.
**Fix:** Wrap `GenealogyTree` (and `FamilyTreeView` if it's the entry point used at all sites) in `next/dynamic(() => import(...), { ssr: false })` at every usage site (`/family-tree`, `/families/[id]`).
**Effort:** 30 min
...
### T-16. Lint regression — `react-hooks/set-state-in-effect` in `PhotoFrame.tsx`

**Found:** 2026-05-08 audit
**File:** `src/components/ui/PhotoFrame.tsx` lines 30-35
**Problem:** The component mirrors the prop `src` into local state inside an effect:
...
React 19's eslint rule flags this because the same state can be derived at render time. It reintroduces the exact warning pattern you already cleaned up on 2026-04-29.
**Fix:** Follow the same render-time prev-prop pattern used in `ProfileAvatar` / `MemoryImage`: track the previous `src`, and when it changes during render, synchronously reset `currentSrc`/`status` before the next commit.
**Effort:** 15 min

### T-17. Webhook new-user route auth spot check

**Found:** 2026-05-08 audit
**File:** `src/app/api/webhooks/new-user/route.ts`
**Problem:** The route is correctly gated by `x-webhook-secret`, but it is one of the remaining service-role routes that deserves a regression test matrix equivalent to the other P0 API fixes. There is no dedicated route test for: missing secret, wrong secret, missing env vars, and happy path skip/send behavior.
**Fix:** Add a focused Vitest suite that mocks `fetch` + `Resend` and pins the auth/error branches. This is a short verification task, not a feature.
**Effort:** 10 min
...
These were the first things I would have Claude Code verify because the earlier agent summary may have overstated completion.
...
## Notes for GPT-4.1 comparison

This TODO list is long-form, strategy-heavy, and opinionated. **Worth comparing against GPT-4.1's take** — different model, different instincts, especially on feature prioritization. Your preferences flagged this pattern. My bias is toward engagement features (voice, reactions, prompts) because research on legacy apps shows most content creation happens in the first 30 days and then drops off a cliff; I'm optimizing for content velocity first, then archival depth.

Also, because this is a **private family app**, I intentionally did **not** push hard on enterprise concerns like RBAC, audit trails, and SOC-style hardening beyond fixing the glaring RLS hole. If you later want this to be multi-family or commercial, reprioritize security + tenancy before adding more features.

---

## Completed Log

- 2026-05-09 — **T-13 Dynamic-import the D3 genealogy tree.** `/family-tree` and [`/families/[id]`](https://github.com/isaacbarcroft/family-tree/pull/50/files) no longer statically pull `d3-selection` / `d3-zoom` into their server-rendered routes. New `src/components/ClientGenealogyTree.tsx` wraps `GenealogyTree` in `next/dynamic(() => import("@/components/GenealogyTree"), { ssr: false, loading: ... })` so the bundle only loads on the client, matching the existing `places/page.tsx` Leaflet pattern. `FamilyTreeView` now renders that client wrapper instead of importing `GenealogyTree` directly, and `/family-tree/page.tsx` does the same, so both entry points stay aligned. Added `src/__tests__/clientGenealogyTree.test.tsx` to pin the no-SSR wrapper and loading fallback, and updated `src/__tests__/treeNode.test.tsx` to keep the tree-node suite isolated from the dynamic wrapper. `TODOS.md` now marks T-13 done and reprioritizes the remaining queue. Verification was limited by the sandbox: `yarn install --frozen-lockfile` could not fetch dependencies because outbound network is blocked, so `yarn lint`, `yarn test`, and `yarn build` could not be run in this workspace.
- 2026-05-06 — **T-12 GenealogyTree SVG node memoization + shared clip-path defs.** Pulled the per-node SVG render out of `src/components/GenealogyTree.tsx` (~150 inline lines, three branches: family-root label / couple / single person) and into a new `React.memo`-wrapped `TreeNode` component at `src/components/TreeNode.tsx`. The memoized `nodes` array, the per-iteration `LayoutNode` references, and the existing `useCallback`-wrapped `navigateToProfile` are all referentially stable across `treeData`-unchanged re-renders, so the inner JSX no longer rebuilds when the parent re-renders for unrelated reasons (today: dims/zoomBehavior state churn). Hoisted the three avatar `<clipPath>` definitions (single, couple-left, couple-right) into one shared `<defs>` block at the top of the SVG. The default `clipPathUnits="userSpaceOnUse"` resolves each clip in the *referencing* element's local coordinate system, so a single `<clipPath>` per variant covers every node, eliminating two `<defs><clipPath>` blocks per couple and one per single person (about 300 fewer SVG nodes on a 200-person tree). Click handlers, deceased styling, name truncation, and the `b. {date}` / `{birth} — {death}` date format are unchanged. Coverage: 11 new Vitest cases (`src/__tests__/treeNode.test.tsx`) cover family-root label rendering, single-person initials and photo variants (including shared `CLIP_ID_SINGLE` reference), single-node click navigation, deceased and living date formats, couple rendering with both halves and the couple-specific clip-paths, a regression pin that no per-node `<defs>` / `<clipPath>` blocks are emitted, click routing on each half of a couple, the shared right-side clip is positioned to the right of the left-side clip, and the memo `displayName` is preserved. 462 tests pass / 5 skipped; lint clean (0 errors / 0 warnings); `yarn build` green. Files: `src/components/TreeNode.tsx` (new), `src/__tests__/treeNode.test.tsx` (new), `src/components/GenealogyTree.tsx`. Deferred (T-12.a real-device frame-rate measurement on a seeded 100/250-person family, depends on T-11.a; T-12.b edge `<path>` memoization, minimal payoff).