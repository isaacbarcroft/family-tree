...
- ~~**1.5.b Half-sibling labeling** ("Half-sibling" when only one parent is shared). Currently labeled as "Sibling". +1h.~~ Done 2026-05-28 on `claude/vigilant-cannon-n1Iup`. See Completed log.
...
- 2026-05-28 — **1.5.b Half-sibling labeling in the relationship calculator.** `findRelationship` previously labeled anyone reachable at `stepsA === 1 && stepsB === 1` (i.e. sharing at least one parent) as a full "Sibling", so half-siblings were shown the wrong relationship on the profile-page chip — a factual error in a genealogy app. Added a pure, exported `classifySiblings(a, b): "full" | "half" | "unknown"` to `src/utils/relationship.ts` that counts shared `parentIds` and returns: `"full"` when ≥2 parents are shared; `"half"` when exactly one parent is shared **and each side has a known distinct other parent** (positive evidence they differ); `"unknown"` otherwise. `bloodLabel` gained an optional `siblingClass` parameter (default `"unknown"`) and now returns `"Half-sibling"` only when the class is `"half"`. `findRelationship` computes the class via `classifySiblings` for the sibling case and threads it through. **Design decision flagged for review:** with incomplete parent data — a child missing its second parent on either side — the label intentionally stays `"Sibling"` rather than downgrading to `"Half-sibling"`, because the missing parent could be the same person (full siblings). Downgrading on absence would assert a relationship the data doesn't support. The alternative (treat "exactly one shared parent" as half regardless of completeness) was rejected for that reason. Coverage: the existing "labels half-siblings as Sibling (documented limitation)" test now expects `"Half-sibling"` (and pins direction symmetry); 3 new collateral-relationship cases (conservative default when one parent on record per child, conservative default when one child is missing its second parent, explicit full-sibling pin); 7 new direct `classifySiblings` unit cases (full at 2 and 3 shared, half, no-shared, one-side-single-parent, no-parents, argument symmetry). `relationship.test.ts` 26 → 36 cases. Full suite 560 pass / 5 skipped; `yarn lint` clean (0/0); `yarn build` green. Files: `src/utils/relationship.ts`, `src/__tests__/relationship.test.ts`, `TODOS.md`. **Deferred:** 1.5.a in-laws, 1.5.c step/adoptive/foster distinctions (needs the `relationships.subtype` join — `parentIds` is denormalized without it), 1.5.d standalone `/relationships` page.
