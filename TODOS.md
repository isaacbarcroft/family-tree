...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ Done 2026-05-07
...
**Outcome:** Migration `20260507_story_prompts.sql` adds `public.story_prompts (id uuid, prompt text unique, category text check in seven values, createdAt timestamptz)` plus a nullable `memories.storyPromptId uuid` back-pointer with `on delete set null` so retiring a prompt does not destroy the memory authored from it. RLS treats the catalog as curated content: only `select` is granted to authenticated users, gated on the existing `is_approved_user()` allowlist helper; no insert/update/delete policy is defined, so only the service role can curate. The migration seeds 56 prompts (8 per category × 7 categories: `childhood`, `career`, `love`, `faith`, `travel`, `holidays`, `pets`) via `insert ... on conflict (prompt) do nothing`, so re-applying the migration or appending more prompts in a future migration is safe and additive. Partial index on `memories.storyPromptId where storyPromptId is not null`. Rollback migration drops the back-pointer column, the policy, both indexes, and the table. New `StoryPrompt` model + `STORY_PROMPT_CATEGORIES` const tuple. New `listStoryPrompts()` and `getStoryPromptById()` helpers in `src/lib/db.ts`. New pure `src/utils/dailyPrompt.ts` exports `localDateKey(date)` (YYYY-MM-DD in the runtime's local TZ), `hashString(input)` (djb2-style, returns a non-negative 32-bit int), and `pickDailyPrompt(prompts, userId, date)` which sorts by id (so the pick is independent of fetch order) and indexes via `hashString(\`${userId}:${localDateKey(date)}\`) % prompts.length`. Result: same (userId, calendar day) returns the same prompt; different days for the same user generally rotate; different users on the same day generally see different prompts. `AddMemoryModal` gains four new optional props (`storyPromptId`, `storyPromptText`, `initialTitle`, `initialDescription`, `autoStartRecording`): when `storyPromptText` is set the modal renders a banner above the title field showing the question being answered, the title field can be prefilled, and `autoStartRecording` kicks off the existing `MediaRecorder` flow on mount (one-shot, guarded by a ref). On save, `addMemory` now receives `storyPromptId` (or `null` when no prompt is in play) so the back-pointer is wired end-to-end. New `DailyPromptCard` component fetches the catalog once per mount, picks today's prompt for the current user, renders a card under the home-page hero ("A question for you today: ...") with **Answer with text** and **Answer with voice** CTAs that open `AddMemoryModal` with the appropriate context (voice CTA passes `autoStartRecording`). The card silently no-ops when the user is signed out, when the catalog is empty, or when the fetch errors, so any failure mode degrades to the existing home page. Wired into `src/app/page.tsx` between the secondary actions and the Getting Started checklist.
**Tests:** 55 new Vitest cases across five files. `storyPromptsMigration.test.ts` (13 cases): table shape, category check covering all seven values, unique constraint on `prompt`, RLS enabled with select-only grant, allowlist-gated select policy, no insert/update/delete policies, `memories.storyPromptId` shape with `on delete set null`, partial index, on-conflict idempotent seed, all categories appear ≥9 times (CHECK + 8 seeds), no policy churn on `memories` (regex tightened so `create index ... on public.memories` doesn't false-positive), no destructive ops, and rollback drops the back-pointer + policy + both indexes + the table. `dailyPrompt.test.ts` (16 cases): `localDateKey` zero-pads, treats two times on the same calendar day as identical, rolls over at midnight; `hashString` returns a non-negative 32-bit int, is deterministic, returns the seed `5381` for the empty string, yields different outputs for nearby inputs; `pickDailyPrompt` returns null on empty catalogs, is stable across calls and across input order, is stable across times of day, rotates across days, spreads across users, returns the only prompt when the catalog has one entry, and never mutates its inputs. `storyPromptsDb.test.ts` (5 cases): `listStoryPrompts` GET path, empty-catalog return, error propagation; `getStoryPromptById` builds an `id=eq.<id>` filter and returns null on a 406. `dailyPromptCard.test.tsx` (8 cases): renders nothing while loading, when signed out (and no fetch fires), when the catalog is empty, or when the fetch errors; renders the prompt + both CTAs after loading; clicking **Answer with text** opens the modal with `storyPromptId`, `storyPromptText`, `autoStartRecording=false`, and a non-empty `initialTitle`; clicking **Answer with voice** sets `autoStartRecording=true`; pin that the same prompt re-renders deterministically across re-mounts. `addMemoryModalStoryPrompt.test.tsx` (5 cases): renders the prompt banner and prefills the title field; forwards `storyPromptId` on the `addMemory` payload; no banner when no prompt prop is supplied; passes `storyPromptId: null` on the payload when no prompt is in play; auto-starts recording when `autoStartRecording=true` and the Record button is replaced by Stop on mount. 512 tests pass / 5 skipped; lint clean (0 errors / 0 warnings); `yarn build` green.
**Follow-ups (deferred):**

- 1.4.a AI follow-up prompts. The original ticket allotted +4h for "AI-generated follow-up prompts based on the user's answer (Claude API call)" — implementing that on top of today's curated catalog is a separate ticket that needs a Claude API key and a content-safety pass. Out of scope here.
- 1.4.b Per-user "skip" / "remind me later" controls. Today the same prompt persists for 24h regardless of whether the user wanted to engage with it. A small `app_users.skippedPromptIds uuid[]` column plus a "Show me a different one" affordance would address that. Estimated +2h.
- 1.4.c Filter prompts by category from the home card. The catalog is already categorized; surfacing a "Pick a category" dropdown lets the user steer toward `pets` or `travel` on a given day. Estimated +1h.
- 1.4.d Show the prompt on the answered memory's detail view. The back-pointer is stored on the memory; the UI does not yet render "Answering: {prompt text}" on `/memories` or on profile-page memory tiles. Estimated +1h.
...
- **GenealogyTree nodes are mouse-only** (`src/components/GenealogyTree.tsx`): Nodes use `onClick` + `cursor: pointer` but have no `tabIndex={0}`, no `role="button"`, no `onKeyDown` handler. Screen reader and keyboard-only users cannot interact with the tree at all. This is the highest-impact a11y gap.
...
- **No focus management after tree node click**: Clicking a node navigates to the profile page but doesn't manage focus — the user lands on the page with no focus indicator.
...
- 2026-05-07 — **1.4 Guided story prompts ("Ask Grandma" mode).** Migration `20260507_story_prompts.sql` adds `public.story_prompts (id, prompt unique, category check in seven values, createdAt)` plus a nullable `memories.storyPromptId` back-pointer with `on delete set null` so retiring a prompt does not destroy the memory authored from it. RLS treats the catalog as curated content: only `select` is granted to authenticated users, gated on the existing `is_approved_user()` allowlist; no insert/update/delete policy is defined, so only the service role can curate. Migration seeds 56 prompts (8 per category × 7 categories: childhood, career, love, faith, travel, holidays, pets) via `insert ... on conflict (prompt) do nothing`, so re-applying the migration or appending more prompts later is safe and additive. Partial index on `memories.storyPromptId` keyed on `where storyPromptId is not null`. Rollback migration drops the back-pointer column, the policy, both indexes, and the table. New `StoryPrompt` model + `STORY_PROMPT_CATEGORIES` const tuple. New `listStoryPrompts()` and `getStoryPromptById()` helpers in `src/lib/db.ts`. New pure `src/utils/dailyPrompt.ts` exports `localDateKey`, `hashString` (djb2-style), and `pickDailyPrompt(prompts, userId, date)` which sorts by id and indexes via `hashString(\`${userId}:${localDateKey(date)}\`) % prompts.length` so the same (userId, calendar day) returns the same prompt; different days rotate; different users on the same day spread. `AddMemoryModal` gains optional props `storyPromptId`, `storyPromptText`, `initialTitle`, `initialDescription`, `autoStartRecording`: when set, the modal renders a banner above the title field showing the question being answered, prefills the title, kicks off the existing `MediaRecorder` flow on mount when `autoStartRecording` is true (one-shot, guarded by a ref), and passes `storyPromptId` (or `null`) into the `addMemory` payload so the back-pointer is wired end-to-end. New `DailyPromptCard` fetches the catalog once per mount, picks today's prompt for the current user, renders a home-page card ("A question for you today: ...") with **Answer with text** and **Answer with voice** CTAs that open `AddMemoryModal` with the right context. Wired into `src/app/page.tsx` between the secondary actions and the Getting Started checklist. The card silently no-ops when the user is signed out, when the catalog is empty, or when the fetch errors. Coverage: 13 migration regex assertions, 16 daily-prompt util cases, 5 db cases, 8 component cases for the card, 5 modal-prefill cases — 47 new Vitest cases total. 512 tests pass / 5 skipped; lint clean (0 errors / 0 warnings); `yarn build` green. Files: `supabase/migrations/20260507_story_prompts.sql` (new), `supabase/migrations/20260507_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/utils/dailyPrompt.ts` (new), `src/components/DailyPromptCard.tsx` (new), `src/__tests__/storyPromptsMigration.test.ts` (new), `src/__tests__/dailyPrompt.test.ts` (new), `src/__tests__/storyPromptsDb.test.ts` (new), `src/__tests__/dailyPromptCard.test.tsx` (new), `src/__tests__/addMemoryModalStoryPrompt.test.tsx` (new), `src/models/Memory.ts`, `src/lib/db.ts`, `src/components/AddMemoryModal.tsx`, `src/app/page.tsx`, `SUPABASE_SETUP.md`, `TODOS.md`. Deferred: 1.4.a AI follow-up prompts (out of scope; needs Claude API key + content review); 1.4.b skip/remind-me-later controls (+2h); 1.4.c category filter on the home card (+1h); 1.4.d show the prompt on the answered memory's detail view (+1h).
