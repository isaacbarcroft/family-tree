...
**In priority order (refreshed 2026-05-26):**
...
3. **1.6.d Run [axe DevTools](https://www.deque.com/axe/devtools/)** and fix all critical / serious issues. Best done after 1.6.b and 1.6.c so the landmark changes are reflected in the report.
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).** Would also upgrade the tree to W3C tree-widget semantics (`role="tree"` / `role="treeitem"` instead of `role="group"` / `role="button"`), since the tree role expects arrow-key roving tabindex.~~ Done 2026-05-26. See Completed log.
...
**Effort:** 2 to 3h remaining across 1.6.b, 1.6.c, 1.6.d.
...
- 2026-05-26 — **1.6.a Arrow-key navigation on the D3 family tree (W3C tree-widget semantics).** Upgraded `src/components/GenealogyTree.tsx` and `src/components/TreeNode.tsx` to the WAI-ARIA tree pattern. The outer `<g>` wrapping all tree content now carries `role="tree"`, `aria-orientation="vertical"`, and an updated `aria-label` that names the arrow-key interaction (`Family tree. Use the arrow keys to move between people, then Enter or Space to open a profile.`). Each focusable person `<g>` is now `role="treeitem"` (was `role="button"`) with `aria-level={depth + 1}` derived from the layout. Roving tabindex: GenealogyTree owns a single `focusedId` state (string | null) that defaults to the first focusable person; the matching treeitem renders `tabindex=0`, every other treeitem renders `tabindex=-1`, so Tab into the tree lands on exactly one person rather than 50 at once. New pure helper `src/utils/treeNavigation.ts` exports `buildFocusables(nodes)` (flattens the layout into `{id, x, y, depth}[]` with two entries per couple and one per single-person node, skipping the synthetic family-root label), `findArrowTarget(key, current, list)` (Up/Down resolves the closest focusable in the previous/next y-row by absolute x-distance, Left/Right finds the next focusable in the same y-row in that x-direction, Home/End jump to first/last), and `isArrowNavigationKey(key)` (gate so non-navigation keys fall through to the browser default — Tab still leaves the tree, characters can still type into nested controls). The tree's `<g>` registers a single `onKeyDown` handler that resolves the target via the helper and, when present, calls `e.preventDefault()` + `setFocusedId(target.id)` and stashes the target in a `pendingFocusRef`; a `useLayoutEffect` consumes the ref after the tabindex update commits and calls `.focus()` on the corresponding SVG group (registered into a `Map<string, SVGGElement>` via a stable `registerNode` callback ref). The initial render does NOT steal focus — `pendingFocusRef` starts as `null` and the effect is a no-op until an arrow key sets it. When `treeData` changes (e.g. on `/families/[id]` navigation), the focusables list is recomputed via `useMemo` and the previously-focused id is re-clamped via the codebase's `prevFocusables` render-time pattern (matching `ProfileAvatar` / `MemoryImage`), so React 19's `react-hooks/set-state-in-effect` warning never fires. Added `depth: number` to `LayoutNode` in `src/utils/treeLayout.ts` so aria-level can be set without reverse-engineering from y. Coverage: 21 new Vitest cases in `src/__tests__/treeNavigation.test.ts` (six `isArrowNavigationKey` keys vs other keys; ArrowDown closest-by-x in next row; ArrowDown on bottom row no-op; ArrowUp on top row no-op; ArrowRight / ArrowLeft within a row; ArrowLeft on leftmost no-op; Home / End; non-navigation key returns undefined; couple-half left↔right round trip; `buildFocusables` single-person / couple / skips synthetic root / depth assignment / empty family). 5 new cases in `src/__tests__/treeNode.test.tsx` (role switched to treeitem, aria-level on single + both couple halves, roving tabindex per couple half, single-person tabindex=-1 when not focused, registerNode wiring for single + both halves), with the pre-existing button-role cases updated to the new treeitem role and to thread the new `focusedPersonId` + `registerNode` props. 8 new cases in `src/__tests__/genealogyTreeA11y.test.tsx` (label group is `role="tree"` with vertical orientation and an arrow-key hint, every interactive person is a nested `role="treeitem"`, exactly one treeitem is tabindex=0 on initial render, ArrowDown moves the roving tabindex into the child row, ArrowRight moves to the right spouse, ArrowLeft returns to the left spouse, ArrowUp returns from a child row to the couple row, Home/End jump to first/last, ArrowDown on the deepest row is a no-op, navigation keys `preventDefault` but Tab does not). 573 tests pass / 5 skipped (was 535/5; +38); `yarn lint` clean (0 / 0); `npx tsc --noEmit` clean; `yarn build` green. Files: `src/components/GenealogyTree.tsx`, `src/components/TreeNode.tsx`, `src/utils/treeLayout.ts`, `src/utils/treeNavigation.ts` (new), `src/__tests__/treeNavigation.test.ts` (new), `src/__tests__/treeNode.test.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `TODOS.md`. Branch: `claude/keen-newton-nhcpQ`. Deferred: 1.6.b (section / article landmarks inside page bodies), 1.6.c (focus management after tree node click), 1.6.d (axe DevTools audit) remain tracked under 1.6. The reset-view button at the top-right of the tree container is still a regular HTML `<button>` outside the role="tree" group, so it is not part of the roving tabindex — keyboard users Tab to it normally as a sibling control.
