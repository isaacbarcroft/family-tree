...
4. **1.4.a** — link memories back to the prompt they answered (`memories.promptId` + a small profile view). First-slice of 1.4 shipped 2026-05-13; this is the natural next step.
...
- ~~Structured oral-history prompts — first slice shipped 2026-05-13 (1.4)~~
...
### ~~1.4. Guided story prompts ("Ask Grandma" mode) — first slice~~ — Done 2026-05-13

See Completed log. The non-AI slice shipped: `story_prompts` table, ~62 seed prompts across all listed categories, deterministic per-UTC-day rotation, home-page widget, and `AddMemoryModal` `initialTitle` pre-fill so the answer flow is one click away. **Open follow-ups:**

- **1.4.a Memory ↔ prompt linkage.** Today the prompt question is pre-filled as the memory title; nothing connects the saved memory back to the `story_prompts` row. A nullable `promptId` column on `memories` + a small "Prompts you've answered" view on the profile / home page is the natural next slice. +2h.
- **1.4.b AI-generated follow-up prompts.** After a memory is saved against a prompt, call Claude (`claude-sonnet-4-6`) with the memory body and ask for two follow-up questions that build on the answer. Needs an admin-gated API key and rate limiting; would slot in after T-6. +4h.
- **1.4.c "Already answered this" suppression.** Once 1.4.a lands, the daily rotation should prefer prompts the viewer has not personally answered (with a graceful fallback once they've answered them all). +1h.
- **1.4.d Per-family or per-user prompt curation.** Today the catalog is curated by hand via the migration; an admin UI to add / archive prompts would let Isaac tailor the set without writing SQL. +2h.
...
- 2026-05-13 — **1.4 Guided story prompts ("Ask Grandma" mode) — first slice.** Shipped the non-AI half of 1.4. Migration `20260513_story_prompts.sql` (+ rollback) adds `public.story_prompts (id, prompt text 1..500, category text check (childhood|career|love|faith|travel|holidays|pets|general), isActive boolean default true, createdAt)`, with a unique index on `prompt` so re-running the seed block is a no-op, a partial index on `isActive = true` for the active-only read path, and a category index for later filtering. RLS gates SELECT on `is_approved_user()` and INSERT / UPDATE / DELETE on `is_admin_user()` — the catalog is curated by hand via migrations, not user input. Seeded 62 prompts across all eight categories (10 childhood, 8 career, 8 love, 6 faith, 6 travel, 8 holidays, 6 pets, 10 general). New `src/models/StoryPrompt.ts` with `StoryPromptCategory` union. New pure `src/utils/storyPrompt.ts` exports `daysSinceEpoch(date)` and `pickPromptForDay(prompts, date)`: filters out inactive rows, sorts the remainder by `id` (so ordering is stable across deploys), and indexes by `floor(daysSinceEpoch(UTC)) % activeCount` so the same prompt shows for everyone in the family on a given UTC day. New `listStoryPrompts()` (PostgREST `is.true` filter; `is.true` not `eq.true` because the custom QueryBuilder's `.eq()` is typed `string` only) and `getStoryPromptForToday(now = new Date())` in `src/lib/db.ts`. New `src/components/StoryPromptWidget.tsx` renders the prompt with an `aria-label="A question for you today"` section heading, a loading skeleton (`aria-busy`), an error path, a `null` short-circuit when the catalog is empty, and an "Answer with a memory" CTA that opens `AddMemoryModal` with the prompt as `initialTitle`. To support that, `AddMemoryModal` gained two new optional props (`initialTitle`, `initialDescription`) that seed the relevant `useState` initial values. Wired the widget into `src/app/page.tsx` between the hero spread and the lower grid. Coverage: 11 migration cases (table shape, prompt-length + category check, RLS gates, indexes, seed format + per-category coverage + ≥ 50-row count, no destructive ops), 1 rollback case, 8 util cases (epoch math, intra-day stability, day-over-day rotation, inactive filtering, empty / single-active paths), 4 db cases (active filter URL, error surfacing, empty-catalog null, per-UTC-day stability), and 5 widget cases (skeleton, resolved render, empty short-circuit, error message, CTA opens modal pre-filled with the prompt title). 558 tests pass / 5 skipped (was 522/5; +36); `yarn lint` clean (0/0); `npx tsc --noEmit` clean; `yarn build` green. `SUPABASE_SETUP.md` lists the new migration. Files: `supabase/migrations/20260513_story_prompts.sql` (new), `supabase/migrations/20260513_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/utils/storyPrompt.ts` (new), `src/components/StoryPromptWidget.tsx` (new), `src/components/AddMemoryModal.tsx`, `src/lib/db.ts`, `src/app/page.tsx`, `src/__tests__/storyPromptsMigration.test.ts` (new), `src/__tests__/storyPrompt.test.ts` (new), `src/__tests__/storyPromptsDb.test.ts` (new), `src/__tests__/storyPromptWidget.test.tsx` (new), `SUPABASE_SETUP.md`, `TODOS.md`. Deferred (1.4.a memory ↔ prompt linkage, 1.4.b AI follow-ups, 1.4.c already-answered suppression, 1.4.d admin curation UI) tracked under 1.4 in the Phase 1 list.
