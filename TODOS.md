...
**In priority order (refreshed 2026-05-31):**

1. **1.6.b Per-section `<section>` / `<article>` landmarks inside page bodies.** Page shells already get `<main>` and `<nav>` via layout; this is the inner-content landmark pass.
2. **1.6.d Run [axe DevTools](https://www.deque.com/axe/devtools/)** and fix all critical / serious issues. Best done after 1.6.b so the landmark changes are reflected in the report.
3. **T-15** — `middleware.ts` for route-level auth. Defense-in-depth on top of RLS. **Re-scope note:** the current `src/lib/supabase.ts` is a hand-rolled client that stores the session in `localStorage`, not in an HTTP cookie. Edge middleware can't read `localStorage`, so the TODOS' 1h estimate undercounts the work — this is either a full migration to `@supabase/ssr` (cookie-based sessions, several hours plus a re-test of every auth path) or a narrower "redirect any request without an `apikey`/`Authorization` cookie to `/login`" pattern that requires the client to first set such a cookie. Worth a separate scoping conversation with Isaac.
...
- Accessibility pass largely done; remaining sub-items are 1.6.b (inner-page landmarks) and 1.6.d (axe DevTools sweep)
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).**~~ Done 2026-05-31. Tree container is now `role="tree"` with an arrow-key usage hint in its `aria-label`; every interactive `<g>` is `role="treeitem"` with `aria-level` and a roving `tabindex` (exactly one item is `0`, all others are `-1`). The container `onKeyDown` routes ArrowUp / ArrowDown / ArrowLeft / ArrowRight / Home / End through `findNextFocusItem`, which finds the geometrically-closest item at the adjacent depth (Up/Down) or the next same-level sibling (Left/Right). Home / End jump to the first / last item. Browser focus on a treeitem (mouse click or programmatic) is mirrored back into the roving state via an `onFocus` handler reading `data-tree-item-id`. See Completed log.
...
- ~~**1.6.c Focus management after tree node click.**~~ Done 2026-05-27 on `claude/vigilant-cannon-CJh08`. New `useFocusOnFirstReady` hook in `src/utils/` lands focus on the profile page's main heading after data load; the hook re-fires on sibling navigation (`/profile/A` → `/profile/B`) but no-ops on intermediate re-renders (e.g. memory list refresh) so focus is not stolen back. The profile page also collapses from two `<h1>` elements (first name + last name on separate lines) to one `<h1>` with two block spans, fixing a pre-existing multiple-h1 violation. See Completed log.
- **1.6.d Run [axe DevTools](https://www.deque.com/axe/devtools/) and fix all critical / serious issues.**

**Effort:** 2 to 3h remaining across 1.6.b and 1.6.d.
...
- 2026-05-31 — **1.6.a Arrow-key navigation on the D3 family tree (W3C tree-widget pattern).** The genealogy tree was mouse-and-Tab-only before this change: Tab cycled through every interactive `<g role="button">` one at a time, which on a 50-person tree meant a screen-reader user pressed Tab dozens of times to reach a great-grandchild. Now the tree implements the WAI-ARIA tree pattern with arrow-key roving tabindex. **Semantics:** `src/components/GenealogyTree.tsx` switches the inner container `<g>` from `role="group"` to `role="tree"` and updates the `aria-label` to advertise the arrow-key affordance ("Use the arrow keys to move between people. Press Enter or Space to open a profile."). `src/components/TreeNode.tsx` switches the three interactive branches (single person, couple-left, couple-right) from `role="button"` to `role="treeitem"` and adds `aria-level={node.level + 1}` (1-based per ARIA). **Roving tabindex:** GenealogyTree owns a `focusedId` state initialized to the first item in pre-order; each TreeNode sets `tabIndex={focusedId === personId ? 0 : -1}` so exactly one treeitem is in the tab cycle at a time. State is rebased to the first valid item when `treeData` changes (render-time `prev*` pattern to preserve the 0/0 `react-hooks/set-state-in-effect` lint baseline). Per-treeitem refs are registered via a stable `registerRef(id, el)` callback into a `Map<string, SVGGElement>` so the parent can call `.focus()` after a state update. **Navigation:** new pure helper `findNextFocusItem(items, currentId, direction)` in `src/utils/treeLayout.ts` resolves the next id. ArrowUp / ArrowDown jump to the geometrically-closest item at the adjacent depth (Euclidean cx distance, strictly-less tie break so the earlier item wins). ArrowLeft / ArrowRight walk to the nearest same-level sibling on the corresponding side. Home / End jump to the first / last item. The parent `<g role="tree">` `onKeyDown` calls `e.preventDefault()` on every arrow / Home / End so the browser doesn't scroll the page, then updates state and focuses the new ref. Non-navigation keys (Enter, Space, Tab, letters) pass through to the existing per-treeitem activation handler in TreeNode, which is unchanged. **Click → roving sync:** a parent `onFocus` handler reads `data-tree-item-id` off the focused element so mouse clicks (which focus tabindex=-1 elements on most browsers) update the roving state, keeping subsequent arrow keys consistent. **Geometry pipeline:** new `collectTreeItems(layout): TreeItem[]` in `treeLayout.ts` walks the layout in pre-order; couples emit two items at the node's quarter-width offsets (so Left/Right can distinguish them by cx), singles emit one at the node center, the synthetic family-root label emits nothing. `LayoutNode` gains a `level: number` field so TreeNode and the items collector both read the same depth. **Coverage:** 15 new cases in `src/__tests__/treeLayoutItems.test.ts` (collectTreeItems on synthetic root / single / couple / mixed depths / label-only nodes; findNextFocusItem for empty input / home / end / unknown currentId fallback / arrow-key cases / edge-of-tree null / same-level filtering / tie break); 11 new cases in `src/__tests__/genealogyTreeKeyboardNav.test.tsx` (initial focus on first item, ArrowDown moves to closest child, multi-step descent, ArrowUp ascent, ArrowLeft / Right same-level walk, Home / End jumps, ignored keys, `preventDefault` on arrows, no preventDefault on other keys, mouse-focus sync, end-to-end roving invariant); 4 updated and 4 added cases in `src/__tests__/treeNode.test.tsx` (role change to treeitem, tabindex=-1 for non-focused, aria-level math at multiple depths, data-tree-item-id, registerRef invocation for single and couple); 2 updated cases in `src/__tests__/genealogyTreeA11y.test.tsx` (role="tree" with arrow-key hint, roving-tabindex invariant). 581 tests pass / 5 skipped (was 540/5; +41); `yarn lint` clean (0 / 0); `yarn build` green. Files: `src/utils/treeLayout.ts`, `src/components/TreeNode.tsx`, `src/components/GenealogyTree.tsx`, `src/__tests__/treeLayoutItems.test.ts` (new), `src/__tests__/genealogyTreeKeyboardNav.test.tsx` (new), `src/__tests__/treeNode.test.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `TODOS.md`. Branch: `claude/keen-newton-3yr7J`. **Known trade-offs:** (1) `aria-setsize` / `aria-posinset` are deliberately omitted — the genealogy model (couples display as two halves at the same node, multi-parent children, etc.) doesn't fit the simple "siblings under one parent" set the spec assumes, and the rendered tree is fully present in the DOM so the SHOULD-but-not-MUST attributes add no screen-reader value. (2) Because the synthetic family-root label sits at layout level 0 but is not a treeitem, the first interactive depth surfaces as `aria-level=2` to assistive tech. Trade-off chosen so the math stays simple for trees both with and without a synthetic root. (3) `React.memo` on TreeNode now re-renders every node on each keypress (focusedId is a new prop). At 250 nodes this is still well under a frame; a per-node `isFocused` boolean prop would preserve memoization but complicates the couple-vs-single API. Deferred unless real-device profiling shows a regression.
- 2026-05-27 — **1.6.c Focus management after route navigation to `/profile/[id]`.** Keyboard and screen-reader users land at the top of the document after a tree-node click (or any `<Link>` to a profile) because nothing on the destination page moved focus — they had to Tab past the NavBar before reaching the person's name. New `src/utils/useFocusOnFirstReady.ts` exposes `useFocusOnFirstReady<T>(ready, key)`, a small hook that returns a ref and calls `.focus({ preventScroll: true })` on the referenced element the first time `ready` flips truthy for a given `key`. Subsequent re-renders while the key is unchanged are no-ops (mid-edit data refresh, memory list updates, etc. do not steal focus back). When `key` changes — e.g. `/profile/A` → `/profile/B` keeps the same route segment but a new id — focus moves again to the new heading. Wired into `src/app/profile/[id]/page.tsx` with `headingRef = useFocusOnFirstReady<HTMLHeadingElement>(!loading && person !== null, person?.id)` and forwarded into the `Hero` component as a new prop. While threading the ref, also collapsed a pre-existing **two-h1 violation**: the page rendered first/middle name on one `<h1>` and last name on a second `<h1>` (the visual design wants two lines), which screen readers announce as two separate document headings. Replaced with a single `<h1 ref={headingRef} tabIndex={-1} aria-label={fullName}>` containing two `<span style={{display: "block"}}>` blocks so the visual treatment is preserved but the document outline now has exactly one h1 per page. `tabIndex={-1}` allows programmatic focus on the non-interactive heading; per CSS spec `:focus-visible` does not match programmatic focus, but defensively added a `.profile-heading:focus, .profile-heading:focus-visible { outline: none }` rule in `globals.css` so the sage focus ring (intended for keyboard tabbing) does not flash on the heading. Coverage: 6 hook cases in `useFocusOnFirstReady.test.tsx` (no-focus while not ready, focus on first ready, no-refocus on same-key re-render, refocus on key change, no-refocus across ready false→true→false→true with same key, undefined→string key transition) and 4 page-level regression-pin cases in `profilePageHeadingFocus.test.ts` reading the page source (exactly one `<h1>`, hook is imported + invoked with the expected signature, `headingRef` is passed to `Hero`, the h1 carries `ref` + `tabIndex={-1}` + `aria-label={fullName}`). 560 tests pass / 5 skipped (was 540/5; +10); `yarn lint` clean (0/0); `yarn build` green. Files: `src/utils/useFocusOnFirstReady.ts` (new), `src/__tests__/useFocusOnFirstReady.test.tsx` (new), `src/__tests__/profilePageHeadingFocus.test.ts` (new), `src/app/profile/[id]/page.tsx`, `src/app/globals.css`, `TODOS.md`. Branch: `claude/vigilant-cannon-CJh08`. **Deferred:** other destination pages (e.g. `/families/[id]`, `/events`, `/memories`, `/timeline`) could adopt the same hook for a consistent pattern, but the TODO scope is specifically "after tree node click" → profile page. Cross-page rollout left for a follow-up.
