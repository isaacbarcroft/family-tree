...
### ~~1.4. Guided story prompts ("Ask Grandma" mode)~~ Done 2026-05-08
...
**Outcome:** Migration `20260508_story_prompts.sql` (+ rollback) creates `public.story_prompts (id, slug, body, category, createdAt, deletedAt)` and seeds it with 56 editorially-curated prompts across the 7 categories called out in the spec plus `general` as the fallback bucket. Inserts are keyed on a unique `slug` with `on conflict do nothing` so re-running the migration is safe and won't clobber admin edits. RLS uses the existing `is_approved_user()` / `is_admin_user()` helpers: SELECT for any approved user, INSERT/UPDATE/DELETE for admins only (this is editorial content, not user-generated). The same migration adds a nullable `promptId uuid` column to `public.memories` referencing `story_prompts(id) on delete set null` so answering a prompt produces a normal `memories` row that carries the back-link; deleting a prompt later keeps the memory. New `StoryPrompt` model + `STORY_PROMPT_CATEGORIES` const tuple. New `listStoryPrompts()` and `listAnsweredPromptIdsForUser(userId)` helpers in `src/lib/db.ts`. Pure helpers in `src/utils/storyPromptPick.ts`: `pickPromptIndex(seed, day, length)` picks a deterministic per-user, per-day index via a djb2-style hash so the displayed prompt is stable for one local-clock day; `filterUnansweredPrompts(prompts, answeredIds)` removes prompts the user has already answered while preserving order; `localDayKey(date)` produces the `YYYY-MM-DD` rotation seed in local time. New `StoryPromptWidget` (`src/components/StoryPromptWidget.tsx`) renders on the signed-in home page above "Getting Started", shows the day's question + category chip, and exposes Answer / Skip buttons. Skip rotates through the unanswered pool; the button disables when only one prompt is left. Answer launches `AddMemoryModal` with a new optional `prompt={ id, body }` prop; the modal swaps its heading to "Answer this prompt", shows the prompt body in an accent-tinted card, prefills the title with the prompt body so the user can edit before saving, and stamps the new memory's `promptId`. Successful save triggers a re-fetch of the pool so the just-answered prompt drops out of rotation immediately. The widget hides itself entirely when the user is not signed in or has answered every available prompt. Coverage: 13 migration regex assertions (table shape, slug uniqueness, eight-value category check, deletedAt + partial active index, RLS gate / admin grants on writes, memories.promptId FK with ON DELETE SET NULL, slug-keyed seed list with `on conflict do nothing`, ≥50 seed rows, pgcrypto, no destructive ops, rollback drops policies/indexes/table/column), 6 db helper cases (filter + ordering, typed result, empty userId short-circuit, scope to user's own non-deleted memories with `select=promptId`, de-dup + drop-nulls, error propagation), 12 pure-utility cases (`pickPromptIndex` empty/range/determinism/cross-day/cross-user, `filterUnansweredPrompts` empty/answered/all-answered, `localDayKey` zero-pad / midnight rollover), 9 widget cases (signed-out null-render, prompt body + heading, answered exclusion, all-answered hide, Skip rotates, single-prompt Skip disabled, Answer opens modal with prompt id + body, modal save triggers re-fetch + skip reset, error fallback), 4 modal-prompt cases (heading swap + prompt-context render, title prefill, promptId on payload, no promptId when no prompt context). 514 tests (509 pass / 5 skip); lint clean (0 errors / 0 warnings); `yarn build` green.

**Follow-ups (deferred — flagged in PR for review):**

- 1.4.a AI-generated follow-up prompts (the optional +4h scope from the original ticket). When a user answers a prompt, send the answer body + the prompt body to the Claude API and ask for one or two follow-up questions ("That's a great memory of your first car — what happened the first time you got pulled over?"); show them inline in the modal as suggestions the user can pick. Out of scope for this PR; needs a server-side Claude API key, a small `/api/ai/follow-up-prompts` route, and a rate-limit decision (T-6 covers rate-limiting infra). Estimated +4h.
- 1.4.b Self-service prompt management UI for admins. Currently prompts are added/edited/deleted through the SQL editor. A `/admin/prompts` page that lists prompts grouped by category with create / edit / soft-delete affordances would let Isaac curate the corpus from inside the app. Estimated +3h.
- 1.4.c Per-prompt "Answer with text" vs "Answer with voice" CTA split. Today the modal exposes both photos and the voice recorder regardless of how the user entered, so a single "Answer" button is enough. If we want the home-page widget to deep-link directly into the recording flow ("Answer with voice"), we'd need a second prop on `AddMemoryModal` to auto-start `MediaRecorder` on mount. Estimated +1h.
- 1.4.d Rotate at a more interesting cadence than once per local day. The current `localDayKey` seed advances at midnight; if we want a "fresh prompt every visit" or "weekly" mode, that's a one-line change in the widget plus a settings hook. Estimated +30min.
...
- 2026-05-08 — **1.4 Guided story prompts ("Ask Grandma" mode).** Migration `20260508_story_prompts.sql` (+ rollback) creates `public.story_prompts (id, slug, body, category, createdAt, deletedAt)` and seeds it with 56 editorially-curated prompts across the 7 spec categories (childhood, career, love, faith, travel, holidays, pets) plus `general` as the fallback bucket. Inserts are keyed on a unique `slug` with `on conflict do nothing` so re-running the migration is safe and won't clobber admin edits. RLS uses the existing `is_approved_user()` / `is_admin_user()` helpers: SELECT for any approved user, INSERT/UPDATE/DELETE for admins only (editorial content, not user-generated). The same migration adds a nullable `promptId uuid` column to `public.memories` referencing `story_prompts(id) on delete set null` so answering a prompt produces a normal `memories` row that carries the back-link; deleting a prompt later preserves the memory. New `StoryPrompt` model + `STORY_PROMPT_CATEGORIES` const tuple. New `listStoryPrompts()` and `listAnsweredPromptIdsForUser(userId)` helpers in `src/lib/db.ts` (the latter filters client-side because the QueryBuilder lacks a `not.is.null` operator; volume per user is modest). Pure helpers in `src/utils/storyPromptPick.ts`: `pickPromptIndex(seed, day, length)` picks a deterministic per-user, per-day index via a djb2-style hash so the displayed prompt is stable for one local-clock day; `filterUnansweredPrompts(prompts, answeredIds)` removes already-answered prompts while preserving order; `localDayKey(date)` produces the `YYYY-MM-DD` rotation seed in local time. New `StoryPromptWidget` (`src/components/StoryPromptWidget.tsx`) renders on the signed-in home page above "Getting Started", shows the day's question + category chip, and exposes Answer / Skip buttons. Skip rotates through the unanswered pool; disables when only one prompt is left. Answer launches `AddMemoryModal` with a new optional `prompt={ id, body }` prop; the modal swaps its heading to "Answer this prompt", shows the prompt body in an accent-tinted context card, prefills the title with the prompt body so the user can edit before saving, and stamps the new memory's `promptId`. Successful save re-fetches the pool so the just-answered prompt drops out of rotation immediately. The widget hides itself entirely when the user is signed-out or has answered every available prompt. Implementation note: the data fetch lives directly inside `useEffect` (not in a `useCallback`) with a `cancelled` flag and a separate `reloadKey` integer for explicit refresh, because the React 19 `react-hooks/set-state-in-effect` rule was flagging the simpler `useCallback`-then-`refresh()` shape used elsewhere in the codebase. Coverage: 13 migration regex assertions, 6 db helper cases, 12 pure-utility cases, 9 widget cases, 4 modal-prompt cases. 514 tests total (509 pass / 5 skip); lint clean (0 errors / 0 warnings); `yarn build` green. Files: `supabase/migrations/20260508_story_prompts.sql` (new), `supabase/migrations/20260508_story_prompts_rollback.sql` (new), `src/models/StoryPrompt.ts` (new), `src/models/Memory.ts`, `src/utils/storyPromptPick.ts` (new), `src/lib/db.ts`, `src/components/StoryPromptWidget.tsx` (new), `src/components/AddMemoryModal.tsx`, `src/app/page.tsx`, `src/__tests__/storyPromptsMigration.test.ts` (new), `src/__tests__/storyPromptsDb.test.ts` (new), `src/__tests__/storyPromptPick.test.ts` (new), `src/__tests__/storyPromptWidget.test.tsx` (new), `src/__tests__/addMemoryModalPrompt.test.tsx` (new), `SUPABASE_SETUP.md`, `TODOS.md`. Deferred: 1.4.a AI-generated follow-up prompts (the optional +4h scope, needs a Claude API key + a `/api/ai/follow-up-prompts` route + rate-limiting from T-6); 1.4.b admin prompt management UI (`/admin/prompts`); 1.4.c per-prompt "Answer with voice" deep-link into the recorder; 1.4.d configurable rotation cadence.
