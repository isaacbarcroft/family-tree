...
**In priority order (refreshed 2026-05-25):**
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).**~~ Done 2026-05-25. Upgraded the tree to W3C tree-widget semantics (`role="tree"` on the inner group, `role="treeitem"` + `aria-level` on each interactive person node) with roving tabindex. Arrow keys move spatially: Left/Right between siblings (couple halves order as left then right), Up to the parent's first focusable, Down to the first child. Home and End jump to the first or last node in the current depth row. Enter and Space activation behavior is preserved. See Completed log.
...
- 2026-05-25 — **1.6.a Arrow-key navigation on the D3 tree with W3C tree-widget semantics.** Largest remaining 1.6 sub-piece. The inner `<g>` wrapping the tree content switched from `role="group"` to `role="tree"`, every interactive person node from `role="button"` to `role="treeitem"`, and roving tabindex is now applied so exactly one focusable node carries `tabindex="0"` at a time; the rest carry `tabindex="-1"`. Each treeitem gains an `aria-level` (1-based) reflecting its depth in the tree. The container's `aria-label` was rewritten to advertise Tab, arrow keys, Home / End, and Enter / Space so the keyboard contract is discoverable from assistive tech. Arrow-key semantics are spatial (more intuitive for a 2D family tree than the W3C tree pattern's flattened-visible-order Up / Down): ArrowLeft and ArrowRight move between siblings in the same depth row (a couple's left half steps to its right half before continuing to the next sibling node), ArrowUp moves to the parent's first focusable, ArrowDown moves to the first focusable in the layout node's children, Home and End jump to the first or last entry in the current row. Out-of-bounds moves (ArrowUp from a top-level person whose parent is the synthetic family-root, ArrowDown from a leaf) are no-ops. Enter and Space still activate (navigate to the person's profile) and `preventDefault` so Space does not scroll the page. Implementation: a new pure focus-index module at `src/utils/treeFocusIndex.ts` exports `buildFocusIndex(layout)` (walks the layout once to emit per-person focus entries with depth + visual centerX + parent-layout map + a per-depth sorted row index + the first-focusable id) and `computeNextFocus(index, fromId, key)` (translates arrow keys to next id with null-safe boundary behavior). A `depth: number` field was added to `LayoutNode` in `src/utils/treeLayout.ts` so `aria-level` and the focus index can compute it without re-deriving from y. `src/components/TreeNode.tsx` now accepts three new optional props (`focusedPersonId`, `registerRef`, `onArrowKey`) used by the parent for roving tabindex, ref registration into the parent's per-id `Map<string, SVGGElement>`, and arrow-key delegation; the activation handler still owns Enter / Space. `src/components/GenealogyTree.tsx` owns the `focusedPersonId` state via the render-time `prev*` pattern (matching the existing `MemoryImage` / `ProfileAvatar` precedent, so the reset on `focusIndex` change lands synchronously instead of via a `useEffect` and dodges `react-hooks/set-state-in-effect`), the per-id ref map, and the arrow-key handler that calls `el.focus()` on the resolved next element. Coverage: 19 new Vitest cases across `src/__tests__/treeFocusIndex.test.ts` (6 builder + 11 computeNextFocus + 2 isArrowNavKey) and `src/__tests__/genealogyTreeKeyboard.test.tsx` (8 end-to-end keyboard cases: initial roving state, ArrowRight sibling step, ArrowDown to first child, ArrowUp back to parent, Home, End, Enter activation routes via `router.push`, ArrowUp boundary no-op at the top-level person). Existing `treeNode.test.tsx` and `genealogyTreeA11y.test.tsx` test suites were updated for the role swap (`button` to `treeitem`, `group` to `tree`) and gained 5 additional cases (aria-level annotation, roving tabindex enforcement, couple-half tabindex split, onArrowKey forwarding with defaultPrevented assertion, non-arrow-key passthrough, registerRef wiring). 573 tests pass / 5 skipped (was 535 / 5; net +38, including +5 in the focus-index suite covered both by new file count and updated test counts inside existing files); `yarn lint` clean (0 errors / 0 warnings); `yarn build` green. Files: `src/utils/treeLayout.ts`, `src/utils/treeFocusIndex.ts` (new), `src/components/TreeNode.tsx`, `src/components/GenealogyTree.tsx`, `src/__tests__/treeNode.test.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `src/__tests__/treeFocusIndex.test.ts` (new), `src/__tests__/genealogyTreeKeyboard.test.tsx` (new), `TODOS.md`. Deferred (call out in PR for review): aria-setsize and aria-posinset on each treeitem (would tell screen readers "person 2 of 3" within a sibling row) are not yet emitted; the existing role + level pair already meets the W3C tree pattern's required attributes, but the optional position hints are a small +1 hour follow-up if a screen-reader pass surfaces the need.
