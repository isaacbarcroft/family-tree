...
### ~~T-14. Webhook new-user route: duplicate PostgREST filter param~~ ✅ Done 2026-05-14
...
- 2026-05-14 — **T-14 Webhook new-user route: collapsed duplicate `email` filter params into a single explicit `and=(...)` clause.** `src/app/api/webhooks/new-user/route.ts` previously assembled the recipient query by appending the `email` query param twice — once with `not.is.null` and once with `neq.<new-user-email>` — and relied on PostgREST treating duplicate keys as AND. That worked in practice but was fragile and non-obvious, and silently bypassed any quoting of the email value (so an address containing reserved characters like `,`, `.`, `(`, `)`, `"`, or `\` could break out of the filter and alter query semantics). The route now builds a single `and=(email.not.is.null,email.neq."<escaped-email>")` clause, with the new user's email wrapped in double quotes and run through the existing `escapePgrstString` helper (`src/utils/pgrstEscape.ts`) so reserved characters can't escape the quoted token. Two new regression tests in `src/__tests__/webhookNewUser.test.ts` parse the URL passed to `fetch` and pin both the new shape (`getAll("email")` is empty, `getAll("and")` matches the assembled clause) and the escape contract (a payload with `"` and `\` in the email round-trips through `URLSearchParams` with the backslash-escaped form intact). Existing 11 webhook cases continue to pass unchanged; 13/13 total. No schema or behavior change for the happy path — PostgREST returns the same recipient set. Files: `src/app/api/webhooks/new-user/route.ts`, `src/__tests__/webhookNewUser.test.ts`, `TODOS.md`. Branch: `claude/vigilant-cannon-uOLCU`.
