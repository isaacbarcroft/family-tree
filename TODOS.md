...
- ~~**1.5.b Half-sibling labeling**~~ — Done 2026-05-13. `findRelationship` now returns `"Half-sibling"` when both people have ≥ 2 recorded parents and share exactly one of them. Stays conservative (`"Sibling"`) when either side has only one known parent so missing data isn't misread as a half-sibling. See Completed log.
...
- 2026-05-13 — **1.5.b Half-sibling labeling.** `src/utils/relationship.ts` previously labeled any two people who shared a single parent as `"Sibling"` — full-sibling and half-sibling pairs were indistinguishable in the profile-page relationship chip. `findRelationship` now classifies the `stepsA === 1, stepsB === 1` case via a new `classifySiblingKind(personA, personB)` helper that inspects both `parentIds` arrays: when both sides have at least two recorded parents and the intersection size is exactly one, the label becomes `"Half-sibling"`; otherwise it stays `"Sibling"`. The conservative gate (requiring two known parents on each side) deliberately avoids downgrading full siblings whose second parent simply hasn't been entered yet — a partial-data row would otherwise be mislabeled as a half-sibling. `bloodLabel` grew a third `siblingKind: "full" | "half"` argument (defaulted to `"full"`) so the classification is computed once in `findRelationship` and threaded through; no other branch of `bloodLabel` is affected. `kind` stays `"blood"` and `commonAncestorId` keeps pointing at the shared parent, which is correct (the LCA on the parent-child graph is unchanged). Coverage: replaced the previously-pinned "documented limitation" case in `src/__tests__/relationship.test.ts` with four new cases — positive half-sibling label (symmetric), conservative "Sibling" when one side has only one known parent, conservative "Sibling" when both sides have only one known parent, and a result-shape assertion confirming `commonAncestorId` / `stepsA` / `stepsB` for the half-sibling path. 530 tests pass / 5 skipped (was 527/5; +3); `yarn lint` clean (0/0); `yarn build` green. Files: `src/utils/relationship.ts`, `src/__tests__/relationship.test.ts`, `TODOS.md`. Still deferred under 1.5: in-law relationships (1.5.a), step/adoptive/foster distinctions (1.5.c, requires schema), standalone `/relationships` picker page (1.5.d).
