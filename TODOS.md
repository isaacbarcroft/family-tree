...
### ~~1.2. Reactions and comments on memories~~ Reactions shipped 2026-04-30; comments + email digest split out as 1.2.b / 1.2.c.
...
#### ~~1.2.a Emoji reactions on memories~~ Done 2026-04-30
**Outcome:** Migration `20260430_memory_reactions.sql` adds `public.memory_reactions (id, memoryId, userId, emoji, createdAt)` with `unique (memoryId, userId, emoji)`, FK cascades on both `memories.id` and `auth.users.id`, and indexes on `memoryId` + `userId`. RLS follows the existing allowlist pattern: SELECT requires `is_approved_user()`, INSERT additionally requires `userId = auth.uid()` (so one approved member can't manufacture reactions for another), and DELETE is restricted to the row's own creator (no admin override; reactions are low-stakes). No UPDATE policy: reactions are immutable, toggling is delete + insert. Rollback migration drops all three policies, both indexes, and the table. New `src/models/MemoryReaction.ts` exports the type plus the canonical `REACTION_EMOJI` tuple (❤️ 😂 🙏 😮 — verbatim from the original ticket). Three new helpers in `src/lib/db.ts`: `listReactionsForMemories(memoryIds[])`, `addReaction(memoryId, userId, emoji)`, `removeReaction(memoryId, userId, emoji)`. New `src/components/MemoryReactions.tsx` renders an accessible reaction bar (`role="group"`, per-button `aria-pressed`, `aria-label` with count and add/remove framing) with optimistic-via-onChange state, error surfacing, and a read-only mode when `currentUserId` is null. Wired into the memories list page (expanded card body, after tagged people) and the profile-page memories grid (under each card). Click events stop propagation so toggling a reaction doesn't collapse the parent card. 24 new Vitest cases across 3 files (`memoryReactionsMigration.test.ts`, `memoryReactionsDb.test.ts`, `memoryReactions.test.tsx`). Lint clean (0 errors / 0 warnings), 286 tests passing total (259 → 286), `yarn build` green.

#### 1.2.b Threaded comments on memories
**Scope:** New table `memory_comments (id, memoryId, userId, body, createdAt, parentCommentId nullable)`. RLS: SELECT for approved members, INSERT requires `userId = auth.uid()`, UPDATE/DELETE only by creator (or admin for moderation). UI: comment thread under each expanded memory card, one level of replies. Need a person/avatar lookup keyed off `userId` since `auth.users` isn't directly readable from policies. Estimated 4–5h on its own.

#### 1.2.c Reaction + comment email digest
**Scope:** Reuse Resend. When someone reacts to or comments on a memory, email the memory's creator (with rate-limit / coalescing to avoid 50 emails for a flurry of reactions). Probably a Supabase Edge Function on a cron, batched daily. Depends on 1.2.b for comments; reactions alone could ship sooner. Estimated 2–3h. Adjacent to 1.3 ("On this day" emails) so worth bundling.
...
- 2026-04-30 — **1.2.a Emoji reactions on memories.** Migration `20260430_memory_reactions.sql` creates `public.memory_reactions` (`id`, `memoryId`, `userId`, `emoji`, `createdAt`) with `unique (memoryId, userId, emoji)`, FK cascades on both `memories.id` and `auth.users.id`, and indexes on `memoryId` + `userId`. RLS: SELECT requires `is_approved_user()`, INSERT additionally requires `userId = auth.uid()`, DELETE only by the row's creator. No UPDATE policy (reactions immutable). Rollback migration included. New `src/models/MemoryReaction.ts` exports the type and the canonical `REACTION_EMOJI` tuple (❤️ 😂 🙏 😮). Three new `src/lib/db.ts` helpers: `listReactionsForMemories`, `addReaction`, `removeReaction`. New `src/components/MemoryReactions.tsx` renders an accessible reaction bar (`role="group"`, per-button `aria-pressed`, `aria-label` reflecting add/remove + count) with optimistic state via `onChange`, error surfacing, and a read-only fallback when `currentUserId` is null. Wired into the memories list page (expanded card) and the profile-page memories grid. 24 new Vitest cases (`memoryReactionsMigration.test.ts`, `memoryReactionsDb.test.ts`, `memoryReactions.test.tsx`). Lint clean, 286 tests passing (259 → 286), `yarn build` green. Comments + email digest deferred as 1.2.b / 1.2.c.
