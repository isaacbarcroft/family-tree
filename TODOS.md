...
**In priority order (refreshed 2026-05-29):**

1. ~~**1.6.a Arrow-key navigation on the D3 tree.**~~ Done 2026-05-29 on `claude/keen-newton-PzuA9`. Tree upgraded to W3C tree-widget semantics (`role="tree"` / `role="treeitem"`, roving tabindex, `aria-level`) with arrow-key navigation. See Completed log.
...
3. **1.6.d Run [axe DevTools](https://www.deque.com/axe/devtools/)** and fix all critical / serious issues. Best done after 1.6.b so the landmark changes are reflected in the report.
...
- ~~**1.6.a Keyboard navigation on the D3 tree (arrow keys to move between nodes, beyond Enter / Space).**~~ Done 2026-05-29 on `claude/keen-newton-PzuA9`. The tree is now a W3C tree widget: the container `<g>` is `role="tree"`, focusable person nodes are `role="treeitem"` (was `role="button"`) with `aria-level` per generation, and a roving tabindex keeps exactly one tab stop for the whole tree. Arrow keys move focus (Up to parent, Down to first child, Left/Right across a generation and between the two halves of a couple), Home/End jump to the first/last person, and Enter/Space still open the profile. Navigation graph lives in a pure, unit-tested helper `src/utils/treeNavigation.ts`. See Completed log. **Deferred:** strict DOM-nested `role="group"` wrappers per generation (depth is currently conveyed via `aria-level` on a flat treeitem list) and `aria-setsize` / `aria-posinset` (optional since every node is present in the DOM).
...
**Effort:** 2 to 3h spread across components for the remaining sub-items (1.6.b landmarks, then the 1.6.d axe pass) now that 1.6.a is shipped.
...
- 2026-05-29 — **1.6.a Arrow-key navigation on the D3 tree (W3C tree-widget upgrade).** The genealogy tree was a flat set of mouse-or-Tab focusable `role="button"` groups (shipped 2026-05-10) wrapped in a `role="group"` container (2026-05-18). Keyboard users had to Tab through every person one at a time, and the tree exposed no widget semantics or spatial navigation. This branch upgrades it to the WAI-ARIA tree pattern. Container `<g>` is now `role="tree"` with an updated usage hint that mentions the arrow keys; every focusable person node is `role="treeitem"` (single-person nodes and each half of a couple) carrying `aria-level` for its generation and a `data-tree-item-id` for focus targeting. A roving tabindex keeps exactly one tab stop for the whole tree (the active item starts at the topmost person and resets there when the tree data changes), so Tab now enters and exits the tree as a single stop. Arrow keys move focus: Up to the parent generation, Down to the first child, Left/Right across a generation (stepping through both spouses of a couple before crossing to the adjacent sibling node), and Home/End to the first/last person in depth-first order; Enter and Space still open the profile, and Space/arrow keys `preventDefault` so the page does not scroll. The navigation graph is computed by a new pure helper `src/utils/treeNavigation.ts` (`buildTreeNavModel` + `nextItemId`), memoized on the layout so the per-node roving props stay referentially stable and `TreeNode`'s `React.memo` still skips the nodes whose active state did not change. Focus is moved by querying the active treeitem's `data-tree-item-id` under the tree's `<g>` ref and calling `.focus()`. Coverage: 13 cases in `src/__tests__/treeNavigation.test.ts` (chain up/down, top-level siblings under a synthetic root, couple half stepping, couple-half-to-sibling bridging, lone single root, synthetic-root omission, and the `nextItemId` arrow/Home/End/unknown-id resolution); `treeNode.test.tsx` updated for the button→treeitem contract plus new cases for roving tabindex, `aria-level`, `data-tree-item-id`, and arrow-key routing; `genealogyTreeA11y.test.tsx` updated for `role="tree"` / `role="treeitem"` and new cases for the single roving tab stop, the initial tab stop on the topmost person, and ArrowDown/ArrowUp moving both the roving tabindex and `document.activeElement`. 567 tests pass / 5 skipped (was 560/5); `yarn lint` clean (0/0); `yarn build` green. Files: `src/utils/treeNavigation.ts` (new), `src/__tests__/treeNavigation.test.ts` (new), `src/components/TreeNode.tsx`, `src/components/GenealogyTree.tsx`, `src/__tests__/treeNode.test.tsx`, `src/__tests__/genealogyTreeA11y.test.tsx`, `TODOS.md`. Branch: `claude/keen-newton-PzuA9`. **Deferred:** strict DOM-nested `role="group"` wrappers per generation (the SVG renders a flat node list positioned by transforms, so depth is conveyed via `aria-level` instead — a recognized flat-tree pattern); `aria-setsize` / `aria-posinset` (optional per APG because every node is present in the DOM); and the 1.6.d axe DevTools pass to confirm no regressions.
