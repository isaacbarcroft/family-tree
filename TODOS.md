...
3. **1.6 Accessibility** — the `TreeNode.tsx` keyboard fix landed 2026-05-11; remaining a11y scope is alt-on-`<img>`, modal focus traps, semantic shell elements, D3 arrow-key tree navigation, and SVG-edge labeling.
...
- ~~**GenealogyTree still mouse-only.**~~ Fixed 2026-05-11. `TreeNode.tsx` interactive `<g>` elements now expose `role="button"`, `tabIndex`, `aria-label`, and Enter/Space activation for both single-person and couple-half nodes.
- **No edge-level auth.** `middleware.ts` does not exist; auth is entirely client-side via `AuthProvider`. Tracked as T-15.
- Accessibility is still partial outside NavBar and the tree (1.6 remaining scope: `<img>` alts, modal focus traps, semantic shells, D3 arrow-key navigation, SVG-edge labels — ~8–10h)
...
- ~~**`TreeNode.tsx` interactive `<g>` elements are mouse-only.** Fixed 2026-05-11. See Completed log.~~
...
- 2026-05-11 — **1.6.a TreeNode keyboard + screen-reader access.** The TODOS audit on 2026-05-08 re-asserted that `src/components/TreeNode.tsx` was the single highest-impact a11y gap: the three interactive `<g>` elements (single-person and the two couple halves) had `onClick` but zero `tabIndex`, `role`, `onKeyDown`, or `aria-label`, so keyboard-only and screen-reader users could not interact with the family tree at all. Fix: each interactive group now sets `role="button"`, `tabIndex={0}` (only when an `id`/`spouseId` is present, otherwise `tabIndex={-1}` with no role/label — the synthetic family-root label stays purely decorative with no role/label/tabindex), an `aria-label` ("View profile of {name}" for couple halves; "View profile of {name}, born {year}" or "View profile of {name}, {birth}–{death}" for single nodes so screen-reader users hear the same lifespan context sighted users see), and an `onKeyDown` that routes Enter and Space to the existing `onNavigate(personId)` and calls `preventDefault()` to suppress Space-scroll. The Enter/Space handler is extracted as a small `activateOnKey` helper at module scope to keep the three call sites identical and testable. No new focus styling needed — `globals.css`'s existing `*:focus-visible { outline: 2px solid var(--sage-deep); outline-offset: 2px }` already covers SVG `<g>` focus indication. New tests (8 cases in `src/__tests__/treeNode.test.tsx`): single-node has `role="button"` + `tabindex="0"` + aria-label containing the name; deceased aria-label includes both years; Enter and Space both fire `onNavigate`; other keys (Tab/letter/Arrow) do not; couples expose two `role="button"` halves with distinct aria-labels and Enter/Space routes each to the correct id; the synthetic family-root group is non-interactive (no role, no aria-label, no tabindex). 525 tests pass / 5 skipped (was 513/5; +12 net, of which 8 are this PR's new cases). `yarn lint` clean (0/0). `yarn build` green. Files: `src/components/TreeNode.tsx`, `src/__tests__/treeNode.test.tsx`, `TODOS.md`. Remaining 1.6 scope (modal focus traps, `<img>` alts, semantic shells, D3 arrow-key navigation between nodes, SVG-edge labels, axe-clean run) is unchanged.
