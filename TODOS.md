...
### ~~T-3. Image optimization~~ ✅ Done 2026-05-31 (server-side thumbnail variant generation deferred)

See Completed log. **Open follow-ups:**

- **T-3.a Avatar (`src/components/ui/Avatar.tsx`) on `next/image`.** Smaller win than `PhotoFrame` (40-48px avatars, mostly inline in NavBar / `/families/[id]` / family-tree card) and the fallback initials path is the common case for users without a profile photo, so deferred. +30 min.
- **T-3.b `ProfileAvatar`'s native-img branch on `next/image`.** The component currently routes Supabase storage URLs through `<img>` and only uses `next/image` for non-Supabase (Firebase) URLs — backwards from where the win lives. Now that the codebase has fully migrated off Firebase, the `useNativeImg` branch is only really needed for `blob:` previews. Consolidate so Supabase URLs go through `next/image` like `PhotoFrame` does. +30 min.
- **T-3.c Server-side thumbnail variants on upload.** Generate small / medium / large derivatives via `sharp` in an edge function or server action so we're not relying on Supabase's image transform endpoint for every thumbnail render. Net win is upload-time CPU spent once vs. per-request transform cost. Tracked but lower priority than the client-side `sizes` win that just shipped. +3h.
...
- 2026-05-31 — **T-3 Extend `next/image` adoption beyond `ProfileAvatar` — migrate `PhotoFrame` to `next/image` with `fill` + tuned `sizes`.** Audit confirmed `MemoryImage` (the wrapper the TODO originally called out) is never rendered outside its own test file — every memory tile, profile portrait, timeline thumbnail, family-tree card, home featured/"in memory" tile, and `/profile/[id]` gallery routes through `src/components/ui/PhotoFrame.tsx`, which still used a plain `<img>`. So `PhotoFrame` is where the win lives. Migrated the internal `<img>` to `next/image` in `fill` mode (the parent already has `position: relative` via `relative overflow-hidden` + an `aspectRatio` style, so `fill` slots in without layout changes). Added two new props: `sizes?: string` (default `"100vw"`, narrowed at every multi-column call site so the browser actually picks a sub-3840px srcset entry instead of the largest), and `priority?: boolean` for above-the-fold LCP candidates (profile-page portrait, home featured memory). `blob:` and `data:` URLs now go through `unoptimized={true}` so the `/_next/image` loader (which validates remote hostnames against `next.config.ts → images.remotePatterns`) doesn't 400 on URLs without a hostname — important if a caller ever passes through an unrevoked `URL.createObjectURL()` preview. Call sites updated with tuned `sizes`: `/memories` collapsed grid (`(min-width: 1024px) 320px, (min-width: 640px) 50vw, 100vw`), `/memories` expanded-card sub-grid (`(min-width: 640px) 240px, 50vw`), `/memories` expanded hero (`(min-width: 768px) 768px, 100vw`), `/profile/[id]` portrait (`(min-width: 1024px) 440px, (min-width: 768px) 360px, 100vw` + `priority`), `/profile/[id]` gallery hero (`(min-width: 1024px) 410px, 100vw`) + rest (`(min-width: 1024px) 290px, 50vw`), home featured (`(min-width: 1024px) 700px, 100vw` + `priority`), home "In memory" tile (`80px`), `/timeline` thumbnail (`64px`), `/family-tree` person card (`(min-width: 1024px) 240px, (min-width: 640px) 33vw, 50vw`). Visual output is unchanged — the only observable difference is in DevTools Network: thumbnails now fetch from `/_next/image?url=...&w=320&q=75` (right-sized) instead of the raw Supabase URL at full resolution, and the browser receives a `srcset` with widths from 32w up to 3840w so it can pick correctly per viewport / DPR. `AddMemoryModal.tsx:359` (the upload-stage `<img>` over a `URL.createObjectURL` blob URL) is left as-is — out of T-3's scope (no remote optimization to gain) and `PhotoFrame`'s `unoptimized` branch would only matter if it were routed through there. Coverage: 7 new regression-pin cases in `src/__tests__/ui/PhotoFrame.test.tsx` (sizes forwards, sizes default `"100vw"`, `priority` drops `loading="lazy"`, lazy by default, `blob:` URL bypasses the optimizer with no srcset, `data:` URL bypasses the optimizer with no srcset, optimized URL emits a `srcset`); the 6 pre-existing cases were retained, with their fixture URLs updated from `example.com` to the realistic `<supabase>.supabase.co/storage/v1/object/public/...` shape (every real call site passes Supabase URLs; this also keeps the `srcset` assertion meaningful since `images.remotePatterns` is built from `NEXT_PUBLIC_SUPABASE_URL`). 557 tests pass / 5 skipped (was 550/5; +7); `yarn lint` clean (0/0); `yarn build` green. Files: `src/components/ui/PhotoFrame.tsx`, `src/__tests__/ui/PhotoFrame.test.tsx`, `src/app/memories/page.tsx`, `src/app/profile/[id]/page.tsx`, `src/app/page.tsx`, `src/app/timeline/page.tsx`, `src/app/family-tree/page.tsx`, `TODOS.md`. Branch: `claude/vigilant-cannon-0wpKk`. **Deferred:** T-3.a `Avatar` migration (smaller surface — 40-48px avatars; fallback-initials path is the common case), T-3.b consolidating `ProfileAvatar`'s `useNativeImg` branch onto `next/image` (its current Supabase-URL fallback to plain `<img>` is backwards — the win lives in optimizing Supabase URLs), T-3.c server-side thumbnail variant generation via `sharp` (much larger scope: edge function + Storage write path + URL convention change; the client-side `sizes` win that just shipped is the cheaper first move).
