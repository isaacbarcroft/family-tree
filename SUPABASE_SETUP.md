...
- `20260528_story_prompts.sql` adds `public.story_prompts` (id, prompt, category, createdAt) for the guided "A question for you today" home widget, seeded with 70 categorized prompts. RLS uses the `app_users` allowlist for SELECT only; INSERT/UPDATE/DELETE are not granted to `authenticated` because prompts are reference data (seed in-migration, curate via the SQL editor). `prompt` is unique and the seed uses `on conflict (prompt) do nothing`, so the migration is safe to re-run. Memories created by answering a prompt store the answer with no foreign key back to this table, so dropping it never orphans a memory. Rollback: `20260528_story_prompts_rollback.sql`.
