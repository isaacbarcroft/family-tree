...
- `20260516_story_prompts.sql` adds `public.story_prompts (id, category, text, createdAt)` for the home-page "Ask Grandma" widget and seeds ~60 open-ended prompts across seven categories (childhood, career, love, faith, travel, holidays, pets). Unique `(category, text)` makes the seed insert idempotent. RLS: SELECT for approved users, write admin-only via `is_admin_user()`. Also adds a nullable `promptId uuid` column to `public.memories` (FK to `story_prompts(id)` on delete set null) so an answered prompt links back to its memory. Rollback: `20260516_story_prompts_rollback.sql`.
