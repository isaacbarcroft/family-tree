...
export async function POST(request: NextRequest) {
  ...
  // Query all people with emails, excluding the new user. Both predicates go
  // into a single PostgREST `and()` filter — appending two `email=...` params
  // also AND-joins, but is fragile (a later `params.set("email", …)` would
  // silently drop the null exclusion). The email is double-quoted + escaped
  // so a value containing `,` or `)` can't break the filter syntax.
  const quotedEmail = `"${escapePgrstString(newUserEmail)}"`
  const params = new URLSearchParams({ select: "email" })
  params.append("and", `(email.not.is.null,email.neq.${quotedEmail})`)
