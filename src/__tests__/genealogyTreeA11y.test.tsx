...
describe("GenealogyTree accessibility", () => {
  ...
  it("keeps interactive person nodes focusable as treeitems inside the labelled tree", () => {
    ...
    const treeitems = treeGroup?.querySelectorAll('g[role="treeitem"]')
    expect(treeitems?.length ?? 0).toBeGreaterThan(0)
    // Roving tabindex: exactly one treeitem carries tabIndex=0.
    const focusable = Array.from(treeitems ?? []).filter(
      (el) => el.getAttribute("tabindex") === "0",
    )
    expect(focusable.length).toBe(1)
    for (const item of Array.from(treeitems ?? [])) {
      const t = item.getAttribute("tabindex")
